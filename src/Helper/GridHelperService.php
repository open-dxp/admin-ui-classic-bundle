...
class GridHelperService
{
        ...
        if ($filterJson) {
            ...
            foreach ($filters as $filter) {
                ...
                if (substr($filterField, 0, 1) == '~') {
                    ...
                    $field = \OpenDxp\Model\DataObject\Classificationstore\Service::getFieldDefinitionFromJson($definition, $type);
                    ...
                    if ($field instanceof ClassDefinition\Data\Objectbricks || $brickDescriptor) {
                        ...
                        if ($filterField == 'fullpath') {
                            ...
                            if ($filter['type'] == 'date' && $operator == '=') {
                                ...
                                // @see \OpenDxp\Model\DataObject\ClassDefinition\Data\Checkbox::getFilterConditionExt()
    ...
    public function prepareListingForGrid(array $requestParams, string $requestedLanguage, User $adminUser): DataObject\Listing\Concrete
    {
        ...
        $sortingSettings = \OpenDxp\Bundle\AdminBundle\Helper\QueryParams::extractSortingSettings($requestParams);
        $doNotQuote = false;
