...
opendxp.registerNS("opendxp.notification.modal");

/**
 * @private
 */
opendxp.notification.modal = Class.create({
    ...
    getWindow: function () {
        if (!this.window) {
            var recipientStore = Ext.create("Ext.data.JsonStore", {
                proxy: {
                    type: "ajax",
                    url: Routing.generate('opendxp_admin_notification_recipients')
                }
            });
            recipientStore.load();
            ...
            var panel = new Ext.form.FormPanel({
                ...
                url: Routing.generate('opendxp_admin_notification_send'),
                items: items,
    ...
    onSuccess: function (form, result, data) {
        opendxp.helpers.showNotification(t("success"), t("notification_has_been_sent"), "success");
        this.getWindow().destroy();
    },
