...
opendxp.registerNS("opendxp.element.properties");
/**
 * @private
 */
opendxp.element.properties = Class.create({
    ...
    getLayout: function () {
        ...
        if (this.layout == null) {
            ...
            var predefinedPropertiesStore = new Ext.data.Store({
                ...
                proxy: {
                    type: 'ajax',
                    url: Routing.generate('opendxp_admin_element_getpredefinedproperties', {elementType: this.type}),
                    reader: {
                        type: 'json',
            ...
            var predefinedcombo = new Ext.form.ComboBox({
                ...
                listClass: "opendxp_predefined_property_select",
                listeners: [{
                            ...
                            handler: function (grid, rowIndex) {
                                ...
                                if(pData.all && pData.all.data) {
                                    if(pData.all.data.id) {
                                        opendxp.helpers.openElement(pData.all.data.id, pData.type, pData.all.data.type);
                                    }
                                    else if (pData.all.data.id) {
                                        opendxp.helpers.openElement(pData.all.data.id, pData.type,
                                                                                        pData.all.data.type);
                                    }
                                }
                            }.bind(this),
            ...
            this.layout = new Ext.Panel({
                ...
                border: false,
                layout: "border",
                iconCls: "opendxp_material_icon_properties opendxp_material_icon",
    ...
    getTypeRenderer: function (value, metaData, record, rowIndex, colIndex, store) {
        ...
        return '<div class="opendxp_icon_' + value + '" name="' + Ext.util.Format.htmlEncode(record.data.name) + '">&nbsp;</div>';
    },
    ...
    getCellRenderer: function (value, metaData, record, rowIndex, colIndex, store) {
        ...
        if (type == "document" || type == "asset" || type == "object") {
            if (value && data.inherited == false) {
                return '<div class="opendxp_property_droptarget">' + value + '</div>';
            }
            else if (data.inherited == false) {
                return '<div class="opendxp_property_droptarget">&nbsp;</div>';
            }
        } else if (type == "bool") {
            if (value) {
        ...
        for (var i = 0; i < rows.length; i++) {
            ...
            try {
                ...
                if (data.inherited == true) {
                    Ext.get(rows[i]).addCls("opendxp_properties_hidden_checkcol");
                }
