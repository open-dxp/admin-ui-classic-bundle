...
opendxp.registerNS("opendxp.element.notes");
/**
 * @private
 */
opendxp.element.notes = Class.create({
    ...
    initialize: function(element, type) {
        ...
        if(element && type) {
            ...
            this.getLayout().on("destroy", function () {
                opendxp.globalmanager.remove("notes");
            });
    ...
    getLayout: function () {
        ...
        if (this.layout == null) {
            ...
            this.store = opendxp.helpers.grid.buildDefaultStore(
                Routing.generate('opendxp_admin_element_notelist'),
            ...
            var tbar = Ext.create('Ext.Toolbar', {
                cls: this.inElementContext ? '' : 'opendxp_main_toolbar',
                items: tbarItems
            });
                ...
                columns.push(
                    {
                        ...
                        items: [{
                            ...
                            handler: function (grid, rowIndex, event) {
                                ...
                                opendxp.helpers.openElement(id, type, null);
                            }.bind(this)
                        }]
                    }
                );
            ...
            var layoutConf = {
                ...
                iconCls: this.inElementContext ? 'opendxp_material_icon_notes opendxp_material_icon' : "opendxp_icon_notes",
                        ...
                        handler: function (grid, rowIndex) {
                            ...
                            if(rec.get("type") == "document" || rec.get("type") == "asset"
                                ...
                                if(rec.get("data") && rec.get("data")["id"]) {
                                    opendxp.helpers.openElement(rec.get("data").id,
    ...
    onAdd: function () {
        ...
        var noteTypesStore = new Ext.data.Store({
            ...
            proxy: {
                type: 'ajax',
                url: Routing.generate('opendxp_admin_element_notetypes', {ctype: this.type}),
                reader: {
                    type: 'json',
        ...
        var addWin = new Ext.Window({
            ...
            buttons: [{
                ...
                handler: function () {
                    ...
                    Ext.Ajax.request({
                        url: Routing.generate('opendxp_admin_element_noteadd'),
                        method: "post",
                        params: values
                    });
    ...
    showDetailedData: function(grid, rowIndex, event) {
        ...
        new opendxp.element.note_details(data.data);
    }
