...
opendxp.registerNS("opendxp.element.scheduler");
/**
 * @private
 */
opendxp.element.scheduler = Class.create({
    ...
    getLayout: function () {
        if (this.layout == null) {
            ...
            if (this.element.data.scheduledTasks.length > 0) {
                ...
                for (var i = 0; i < this.element.data.scheduledTasks.length; i++) {
                    ...
                    td = [
                        rawTask.id,
                        d,
                        Ext.Date.format(d, opendxp.globalmanager.get('localeDateTime').getShortTimeFormat()),
                        rawTask.action
                    ];
            ...
            var storeFields = [
                "id",
                {
                    ...
                    convert: function (v, rec) {
                        ...
                        if (v instanceof Date) {
                            ret = Ext.Date.format(v, opendxp.globalmanager.get('localeDateTime').getShortDateFormat());
                        }
                        return ret;
                    }
                },
                {
                    ...
                    convert: function (v, rec) {
                        var ret = v;
                        if (v instanceof Date) {
                            ret = Ext.Date.format(v, opendxp.globalmanager.get('localeDateTime').getShortTimeFormat());
                        }
                        return ret;
                    }
                },
            ...
            if (this.options.supportsVersions) {
                this.versions = new Ext.data.Store({
                    ...
                    fields: ['id', {name: 'date', convert: function (v, rec) {
                            ...
                            var ret = Ext.Date.format(d, opendxp.globalmanager.get('localeDateTime').getShortDateTimeFormat());
            ...
            this.layout = new Ext.Panel({
                ...
                border: false,
                iconCls: "opendxp_material_icon_scheduler opendxp_material_icon",
                items: [this.grid],
                layout: "fit"
            });
        }
    ...
    getValues: function () {
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            let dateTime = Ext.Date.parseDate(dateString, opendxp.globalmanager.get('localeDateTime').getShortDateTimeFormat());
            value = {
