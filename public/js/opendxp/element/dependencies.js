...
opendxp.registerNS("opendxp.element.dependencies");
/**
 * @private
 */
opendxp.element.dependencies = Class.create({
    ...
    getLayout: function() {
        ...
        if (this.layout == null) {
            this.layout = new Ext.Panel({
                ...
                iconCls: "opendxp_material_icon_dependencies opendxp_material_icon",
                listeners:{
    ...
    getRequiresLayout: function() {
        ...
        this.requiresStore = new Ext.data.Store({
            ...
            proxy : {
                type: 'ajax',
                url: Routing.generate('opendxp_admin_element_getrequiresdependencies'),
                reader: {
                    type: 'json',
        ...
        this.requiresGrid = new Ext.grid.GridPanel({
                ...
                {text: t("subtype"), sortable: true, dataIndex: 'subtype', width: 50,
                  renderer:
                    function (value, metaData, record, rowIndex, colIndex, store) {
                        return '<div style="height: 16px;" class="opendxp_icon_' + record.get('type') + ' opendxp_icon_' + value
            ...
            columnLines: true,
            stripeRows: true,
            bbar: opendxp.helpers.grid.buildDefaultPagingToolbar(this.requiresStore, {pageSize: itemsPerPage})
        });
    ...
    getRequiredByLayout: function() {
        ...
        this.requiredByStore = new Ext.data.Store({
            ...
            proxy : {
                type: 'ajax',
                url: Routing.generate('opendxp_admin_element_getrequiredbydependencies'),
                reader: {
                    type: 'json',
        ...
        this.requiredByGrid = Ext.create('Ext.grid.Panel', {
            ...
            stripeRows: true,
            flex: 1,
            bbar: opendxp.helpers.grid.buildDefaultPagingToolbar(this.requiredByStore,{pageSize: itemsPerPage})
        });
    ...
    click: function ( grid, record, tr, rowIndex, e, eOpts ) {
        var d = record.data;
        opendxp.helpers.openElement(d.id, d.type, d.subtype);
    },
    ...
    onRowContextmenu: function (grid, record, tr, rowIndex, e, eOpts) {
        ...
        menu.add(new Ext.menu.Item({
            ...
            handler: function (data) {
                opendxp.helpers.openElement(data.id, data.type, data.subtype);
            }.bind(this, data)
        }));
