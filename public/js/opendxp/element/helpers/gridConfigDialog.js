...
opendxp.registerNS("opendxp.element.helpers.gridConfigDialog");
/**
 * @private
 */
opendxp.element.helpers.gridConfigDialog = Class.create({
    ...
    getSaveAndSharePanel: function () {
        ...
        if (user.isAllowed("share_configurations")) {
            this.userStore = new Ext.data.JsonStore({
                ...
                proxy: {
                    type: 'ajax',
                    url: Routing.generate('opendxp_admin_user_getusersforsharing'),
                    reader: {
                        rootProperty: 'data',
                        idProperty: 'id'
                    }
            ...
            this.rolesStore = new Ext.data.JsonStore({
                ...
                proxy: {
                    type: 'ajax',
                    url: Routing.generate('opendxp_admin_user_getrolesforsharing'),
                    reader: {
                        rootProperty: 'data',
                        idProperty: 'id'
                    }
        ...
        this.settingsForm = Ext.create('Ext.form.FormPanel', {
            ...
            autoScroll: true,
            border: false,
            iconCls: "opendxp_icon_save_and_share",
    ...
    getLanguageSelection: function (config) {
        ...
        for (let i = 0; i < opendxp.settings.websiteLanguages.length; i++) {
            storedata.push([opendxp.settings.websiteLanguages[i],
        ...
        this.itemsPerPage = new Ext.form.ComboBox({
            ...
            autoSelect: true,
            editable: true,
            value: (this.config.pageSize ? this.config.pageSize : opendxp.helpers.grid.getDefaultPageSize(-1)),
            store: new Ext.data.ArrayStore({
                id: 0,
                fields: [
                    'id',
        ...
        if (this.previewSettings.showPreviewSelector && opendxp.helpers.hasSearchImplementation()) {
            items.push({
                xtype: "button",
                text: t("preview_item"),
    ...
    openSearchEditor: function () {
        opendxp.helpers.itemselector(false, this.applyPreviewItem.bind(this), {
    ...
    getOperatorTree: function (operatorGroupName, groups) {
        ...
        var tree = new Ext.tree.TreePanel({
            ...
            iconCls: 'opendxp_icon_gridconfig_operator_' + operatorGroupName,
            xtype: "treepanel",
            region: "south",
            autoScroll: true,
            layout: 'fit',
