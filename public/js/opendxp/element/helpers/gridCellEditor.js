 ...
 * @private
 */
Ext.define('opendxp.element.helpers.gridCellEditor', {
    ...
    startEdit: function(el, value, /* private: false means don't focus*/
        ...
        if (fieldInfo.layout.noteditable) {
            opendxp.helpers.showNotification(t('warning'), t('this_element_cannot_be_edited'), 'warning');
            return;
        }
        ...
        if (fieldType == "assetmetadata") {
            tag = new opendxp.asset.metadata.tags[tagType](value, fieldInfo.layout);
        } else {
            tag = new opendxp[fieldType].tags[tagType](value, fieldInfo.layout);
        }
