...
opendxp.registerNS("opendxp.element.tag.tree");
/**
 * @private
 */
opendxp.element.tag.tree = Class.create({
    ...
    getLayout: function () {
        if (!this.tree) {
            ...
            var store = Ext.create('Ext.data.TreeStore', {
                ...
                proxy: {
                    type: 'ajax',
                    url: Routing.generate('opendxp_admin_tags_treegetchildrenbyid'),
                    extraParams: {
                ...
                viewConfig: {
                    ...
                    listeners: {
                        drop: function (node, data, overModel, dropPosition, eOpts) {
                            ...
                            Ext.Ajax.request({
                                url: Routing.generate('opendxp_admin_tags_update'),
                                method: 'PUT',
                                params: {
            ...
            menu.add(new Ext.menu.Item({
                ...
                handler: function (tree, record) {
                    Ext.Msg.confirm(t('delete'), sprintf(t('delete_message_advanced'), t('tag'), record.data.text), function (btn) {
                        if (btn == 'yes') {
                            Ext.Ajax.request({
                                url: Routing.generate('opendxp_admin_tags_delete'),
                                method: 'DELETE',
                                params: {
                                    id: record.data.id
                                },
    ...
    addTagComplete: function (tree, record, button, value, object) {
        ...
        if (button == "ok" && value.length > 0) {
            ...
            Ext.Ajax.request({
                url: Routing.generate('opendxp_admin_tags_add'),
                method: 'POST',
                params: {
                ...
                success: function (tree, record, response) {
                    ...
                    if (!res.success) {
                        opendxp.helpers.showNotification(t("error"), t("error"), "error", t(res.message));
                    }
