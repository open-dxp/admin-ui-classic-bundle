...
function t(key, defaultValue, placeholders) {
    ...
    var alreadyTranslated = opendxp.globalmanager.get("translations_admin_translated_values");
    ...
    if (opendxp && opendxp.system_i18n && (opendxp.system_i18n[key] || opendxp.system_i18n[originalKey])) {
        var trans = opendxp.system_i18n[originalKey] ? opendxp.system_i18n[originalKey] : opendxp.system_i18n[key];
        ...
        opendxp.globalmanager.get("translations_admin_translated_values").push(trans);
        return trans;
    }

    var transKeys = opendxp && opendxp.system_i18n ? Object.keys(opendxp.system_i18n) : {};
    if(opendxp && opendxp.system_i18n && transKeys.indexOf(key) === -1 && transKeys.indexOf(originalKey) === -1){
        if(!defaultValue && !in_array(key, alreadyTranslated)) {
            if(opendxp.globalmanager.exists("translations_admin_missing")) {
                if (!in_array(key, opendxp.globalmanager.get("translations_admin_added")) &&
                    !in_array(key, opendxp.globalmanager.get("translations_admin_missing"))) {
                    opendxp.globalmanager.get("translations_admin_missing").push(key);
                }
            }
        }
    }

    if(parent.opendxp && parent.opendxp.settings.debug_admin_translations){ // use parent here, because it's also used in the editmode iframe
        return "+" + key + "+";
...
function dateToServerTimezone(date) {
    ...
    let utcDate = new Date(date.toLocaleString('en-US', {
        timeZone: opendxp.settings.timezone ? opendxp.settings.timezone : 'UTC'
    }));
