...
opendxp.registerNS("opendxp.document.pages.settings");
/**
 * @private
 */
opendxp.document.pages.settings = Class.create(opendxp.document.settings_abstract, {
    ...
    getLayout: function () {
        ...
        if (this.layout == null) {
            ...
            this.layout = new Ext.FormPanel({
                ...
                border: false,
                autoScroll: true,
                iconCls: "opendxp_material_icon_page_settings opendxp_material_icon",
                ...
                items: [
                    {
                        ...
                        items :[
                            ...
                            },
                            {
                                xtype: "container",
                                itemId: "serpPreview",
                                cls: "opendxp_document_page_serp_preview",
                                hidden: true,
                                html:
                        ...
                        items :[
                            {
                                ...
                                listeners: {
                                    "focusleave": function (el) {
                                        Ext.Ajax.request({
                                            url: Routing.generate('opendxp_admin_document_page_checkprettyurl'),
                                            method: "POST",
                                            params: {
                                                id: this.document.id,
                                                path: opendxp.helpers.sanitizeUrlSlug(el.getValue())
                                            },
                                            success: function (res) {
                                                res = Ext.decode(res.responseText);
                                                if(!res.success) {
                                                    el.getEl().addCls("opendxp_error_input");
                                                    if (res.message) {
                                                        Ext.MessageBox.alert(t("info"), res.message);
                                                    }
                                                } else {
                                                    el.getEl().removeCls("opendxp_error_input");
                                                }
                                            }
                                        });
                                    }.bind(this),
                                    "change": function (el) {
                                        const sanitizedValue = opendxp.helpers.sanitizeUrlSlug(el.getValue());
            ...
            const additionalSettings = new CustomEvent(opendxp.events.prepareDocumentPageSettingsLayout, {
                detail: {
                   layout: this.layout,
