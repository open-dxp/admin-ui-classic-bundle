...
opendxp.registerNS("opendxp.document.editables.areablock");
/**
 * @private
 */
opendxp.document.editables.areablock = Class.create(opendxp.document.area_abstract, {
    ...
    refresh: function() {
        this.elements = Ext.get(this.id).query('.opendxp_block_entry[data-name="' + this.name + '"][key]');
        ...
        if (this.elements.length < 1) {
            ...
            for (var i = 0; i < this.elements.length; i++) {
                ...
                var buttonContainer = Ext.get(this.elements[i]).selectNode('.opendxp_area_buttons', false);
                ...
                if(this.config['controlsTrigger'] === 'hover') {
                    Ext.get(this.elements[i]).on('mouseenter', function (event) {
                        ...
                        var buttonContainer = Ext.get(event.target).selectNode('.opendxp_area_buttons', false);
                    ...
                    Ext.get(this.elements[i]).on('mouseleave', function (event) {
                        hideTimeout = window.setTimeout(function () {
                            Ext.get(event.target).selectNode('.opendxp_area_buttons', false).hide();
                            hideTimeout = null;
                        }, 10000);
                    });
                }
    ...
    refreshControls: function (element, limitReached) {
        ...
        plusUpButton = Ext.get(element).query('.opendxp_block_plus_up[data-name="' + this.name + '"] .opendxp_block_button_plus', false)[0];
        if (typeof plusUpButton !== 'undefined') {
            plusUpButton.remove();
        }

        plusButton = Ext.get(element).query('.opendxp_block_plus[data-name="' + this.name + '"] .opendxp_block_button_plus', false)[0];
        ...
        if(!limitReached) {
            // plus buttons
            plusUpDiv = Ext.get(element).query('.opendxp_block_plus_up[data-name="' + this.name + '"]')[0];
            plusUpButton = new Ext.Button({
                cls: "opendxp_block_button_plus",
                iconCls: "opendxp_icon_plus_up",
                arrowVisible: false,
                menu: this.getTypeMenu(this, element, "before")
            });
            plusUpButton.render(plusUpDiv);

            plusDiv = Ext.get(element).query('.opendxp_block_plus[data-name="' + this.name + '"]')[0];
            plusButton = new Ext.Button({
                cls: "opendxp_block_button_plus",
                iconCls: "opendxp_icon_plus_down",
                arrowVisible: false,
        ...
        // minus button
        minusButton = Ext.get(element).query('.opendxp_block_minus[data-name="' + this.name + '"] .opendxp_block_button_minus')[0];
        if (typeof minusButton === 'undefined') {
            minusDiv = Ext.get(element).query('.opendxp_block_minus[data-name="' + this.name + '"]')[0];
            minusButton = new Ext.Button({
                cls: "opendxp_block_button_minus",
        ...
        // up button
        upButton = Ext.get(element).query('.opendxp_block_up[data-name="' + this.name + '"] .opendxp_block_button_up')[0];
        if (typeof upButton === 'undefined') {
            upDiv = Ext.get(element).query('.opendxp_block_up[data-name="' + this.name + '"]')[0];
            upButton = new Ext.Button({
                cls: "opendxp_block_button_up",
                iconCls: "opendxp_icon_white_up",
                listeners: {
        ...
        // down button
        downButton = Ext.get(element).query('.opendxp_block_down[data-name="' + this.name + '"] .opendxp_block_button_down')[0];
        if (typeof downButton === 'undefined') {
            downDiv = Ext.get(element).query('.opendxp_block_down[data-name="' + this.name + '"]')[0];
            downButton = new Ext.Button({
                cls: "opendxp_block_button_down",
                iconCls: "opendxp_icon_white_down",
                listeners: {
        ...
        // type button
        typeButton = Ext.get(element).query('.opendxp_block_type[data-name="' + this.name + '"] .opendxp_block_button_type')[0];
        if (typeof typeButton === 'undefined') {
            typeDiv = Ext.get(element).query('.opendxp_block_type[data-name="' + this.name + '"]')[0];
            typeButton = new Ext.Button({
                cls: "opendxp_block_button_type",
                handleMouseEvents: false,
                tooltip: t("drag_me"),
                iconCls: "opendxp_icon_white_move",
                style: "cursor: move;"
            });
        ...
        // option button
        optionsButton = Ext.get(element).query('.opendxp_block_options[data-name="' + this.name + '"] .opendxp_block_button_options')[0];
        if (typeof optionsButton === 'undefined') {
            optionsDiv = Ext.get(element).query('.opendxp_block_options[data-name="' + this.name + '"]')[0];
            optionsButton = new Ext.Button({
                cls: "opendxp_block_button_options",
                iconCls: "opendxp_icon_white_copy",
                listeners: {
        ...
        if (typeof visibilityButtons === 'undefined') {
            var elementIcon = "opendxp_material_icon_preview";
            if (element.dataset.hidden == "true") {
                elementIcon = "opendxp_icon_white_hide";
            }
            visibilityDiv = Ext.get(element).query('.opendxp_block_visibility[data-name="' + this.name + '"]')[0];
            this.visibilityButtons[element.key] = new Ext.Button({
                cls: "opendxp_block_button_visibility",
                iconCls: elementIcon,
                enableToggle: true,
                pressed: (element.dataset.hidden == "true"),
                toggleHandler: function (element, el) {
                    Ext.get(element).toggleCls('opendxp_area_hidden');
                    if (el.btnIconEl.dom.classList.contains('opendxp_icon_white_hide')) {
                        Ext.get(el.btnIconEl.dom).removeCls('opendxp_icon_white_hide');
                        Ext.get(el.btnIconEl.dom).addCls('opendxp_material_icon_preview');
                    } else {
                        Ext.get(el.btnIconEl.dom).removeCls('opendxp_material_icon_preview');
                        Ext.get(el.btnIconEl.dom).addCls('opendxp_icon_white_hide');
                    }
                }.bind(this, element)
            });
            ...
            if(element.dataset.hidden == "true") {
                Ext.get(element).addCls('opendxp_area_hidden');
            }
        }


        //dialogBox button
        dialogBoxDiv = Ext.get(element).query('.opendxp_block_dialog[data-name="' + this.name + '"]')[0];
        if (dialogBoxDiv) {
            dialogBoxButton = Ext.get(element).query('.opendxp_block_dialog[data-name="' + this.name + '"] .opendxp_block_button_dialog')[0];
        ...
        labelDiv = Ext.get(Ext.get(element).query('.opendxp_block_label[data-name="' + this.name + '"]')[0]);
    ...
    applyFallbackIcons: function() {
        ...
        if (this.config.types) {
            for (var i = 0; i < this.config.types.length; i++) {
                ...
                if (!brick.icon) {
                    brick.icon = "/bundles/opendxpadmin/img/flat-color-icons/" + iconStore[i + 1] + ".svg";
                }
            }
        }
    },
    ...
    copyToClipboard: function (element) {
        ...
        opendxp.globalmanager.add("areablock_clipboard", Ext.encode(item));
    },
    ...
    optionsClickhandler: function (element, btn, e) {
        ...
        if(opendxp.globalmanager.exists("areablock_clipboard")) {
            menu.add(new Ext.menu.Item({
                text: t('paste'),
                iconCls: "opendxp_icon_paste",
                handler: function (item) {
                    item.parentMenu.destroy();
                    item = opendxp.globalmanager.get("areablock_clipboard");
                    /*
                    ...
                    // push the data as an object compatible to the opendxp.document.editable interface to the rest of
                    // available editables so that they get read by opendxp.document.edit.getValues()
    ...
    setInherited: function ($super, inherited) {
        var elements = Ext.get(this.id).query('.opendxp_area_buttons[data-name="' + this.name + '"]');
    ...
    startDragDrop: function () {
        Ext.getBody().addCls('opendxp_drag_drop_active');
        Ext.get(this.id).addCls('opendxp_editable_areablock_dropzones_active');
    },

    endDragDrop: function () {
        Ext.getBody().removeCls('opendxp_drag_drop_active');
        Ext.get(this.id).removeCls('opendxp_editable_areablock_dropzones_active');
    },
    ...
    updateDropZones: function () {
        ...
        var dropZones = Ext.get(this.id).query("div.opendxp_area_dropzone");
    ...
    addDropZoneToElement: function (el) {
        el.dropZone = new Ext.dd.DropZone(el, {
            ...
            onNodeEnter : function(target, dd, e, data){
                Ext.fly(target).addCls('opendxp_area_dropzone_hover');
            },

            onNodeOut : function(target, dd, e, data){
                Ext.fly(target).removeCls('opendxp_area_dropzone_hover');
            },
    ...
    createInitalControls: function () {
        ...
        plusEl.setAttribute("class", "opendxp_block_plus");
        plusEl.setAttribute("data-name", this.name);

        var optionsEl = document.createElement("div");
        optionsEl.setAttribute("class", "opendxp_block_options");
        optionsEl.setAttribute("data-name", this.name);

        var clearEl = document.createElement("div");
        clearEl.setAttribute("class", "opendxp_block_clear");
    ...
    getMenuConfigForBrick: function (brick, scope, element, insertPosition) {
        ...
        if (brick.limit) {
            menuText += ' <span class="opendxp_areablock_menu_limit">('+ brick.limit +')</span>';
        }
    ...
    addBlockAt: function (type, index, forceReload) {
        ...
        if(this.config.types[brickIndex]['needsReload'] || forceReload === true || this.config.reload === true) {
            editWindow.lastScrollposition = '#' + this.id + ' .opendxp_block_entry[data-name="' + this.name + '"][key="' + nextKey + '"]';
            ...
            Ext.Ajax.request({
                url: Routing.generate('opendxp_admin_document_page_areabrick-render-index-editmode'),
                method: 'post',
                params: {
    ...
    removeBlock: function (element) {
        ...
        let dialogBoxDiv = container.query('.opendxp_block_dialog[data-name="' + this.name + '"]')[0];
        if (dialogBoxDiv) {
    ...
    createToolBar: function () {
        ...
        if(opendxp.document.editables[this.toolbarGlobalVar] != false
                                                && opendxp.document.editables[this.toolbarGlobalVar].itemCount) {
            itemCount = opendxp.document.editables[this.toolbarGlobalVar].itemCount;
        }
        ...
        if(typeof this.config.group != "undefined") {
            ...
            for (var g=0; g<groups.length; g++) {
                ...
                if(groups[g].length > 0) {
                    ...
                    if(opendxp.document.editables[this.toolbarGlobalVar] != false) {
                        if(opendxp.document.editables[this.toolbarGlobalVar]["groups"][groups[g]]) {
                            groupMenu = opendxp.document.editables[this.toolbarGlobalVar]["groups"][groups[g]];
        ...
        if(opendxp.document.editables[this.toolbarGlobalVar] == false) {
            ...
            opendxp.document.editables[this.toolbarGlobalVar] = {
                toolbar: toolbar,
            ...
            opendxp.document.editables[this.toolbarGlobalVar].toolbar.add(buttons);
            opendxp.document.editables[this.toolbarGlobalVar].bricks =
                                    array_merge(opendxp.document.editables[this.toolbarGlobalVar].bricks, bricksInThisArea);
            opendxp.document.editables[this.toolbarGlobalVar].groups =
                                    array_merge(opendxp.document.editables[this.toolbarGlobalVar].groups, groupsInThisArea);
            opendxp.document.editables[this.toolbarGlobalVar].itemCount += buttons.length;
            opendxp.document.editables[this.toolbarGlobalVar].areablocks.push(this);
            opendxp.document.editables[this.toolbarGlobalVar].toolbar.updateLayout();
        }
    ...
    getToolBarButton: function (brick, key, itemCount, type) {
        ...
        if(opendxp.document.editables[this.toolbarGlobalVar] != false) {
            if(in_array(key, opendxp.document.editables[this.toolbarGlobalVar].bricks)) {
                return;
            }
        }
        ...
        var button = {
            ...
            textAlign: "left",
            icon: brick.icon,
            cls: 'opendxp_cursor_move',
            text: (brick.name.length > maxButtonCharacters ? brick.name.substr(0,maxButtonCharacters) + "..."
                : brick.name) + (brick.limit ? ' <span class="opendxp_areablock_menu_limit">('+ brick.limit +')</span>' : ''),
            width: areaBlockToolbarSettings.buttonWidth,
            handler: function () {
                ...
                "afterrender": function (brick, v) {
                    ...
                    v.dragZone = new Ext.dd.DragZone(v.getEl(), {
                        ...
                        onStartDrag: function () {
                            ...
                            // create drop zones
                            var areablocks = opendxp.document.editables[this.toolbarGlobalVar].areablocks;
...
opendxp.document.editables.areablocktoolbar = false;
