...
opendxp.registerNS("opendxp.document.editables.scheduledblock");
/**
 * @private
 */
opendxp.document.editables.scheduledblock = Class.create(opendxp.document.editables.block, {

    initialize: function($super, id, name, config, data, inherited) {
        $super(id, name, config, data, inherited);

        this.elements = Ext.get(id).query('.opendxp_block_entry[data-name="' + name + '"][key]');
        ...
        Ext.get(id).on('mouseenter', function (event) {
            Ext.get(id).addCls('opendxp_block_entry_over');
        });

        Ext.get(id).on('mouseleave', function (event) {
            Ext.get(id).removeCls('opendxp_block_entry_over');
        });
    ...
    getTmpStoreId: function() {
        ...
        return "opendxp_scheduled_block_tmp_date_" + documentId + "_" + this.name;
    },

    renderControls: function() {

        var controlDiv = Ext.get(this.id).query('.opendxp_scheduled_block_controls')[0];

        var controlItems = [];

        var initialDate = new Date();
        if(top.opendxp.globalmanager.get(this.getTmpStoreId())) {
            initialDate = top.opendxp.globalmanager.get(this.getTmpStoreId());
            top.opendxp.globalmanager.remove(this.getTmpStoreId());
        }
        ...
        this.slider = Ext.create('Ext.opendxp.slider.Milestone', {
            width: '100%',
            region: 'center',
        ...
        for (var i = 0; i < this.elements.length; i++) {
            ...
            jumpMenuEntries.push({
                text: Ext.Date.format(timestamp, opendxp.globalmanager.get('localeDateTime').getShortDateTimeFormat()),
                iconCls: 'opendxp_icon_time',
    ...
    addBlock : function (date) {
        ...
        opendxp.globalmanager.add(this.getTmpStoreId(),  date);
