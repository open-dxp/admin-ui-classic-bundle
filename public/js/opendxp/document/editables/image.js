...
opendxp.registerNS("opendxp.document.editables.image");
/**
 * @private
 */
opendxp.document.editables.image = Class.create(opendxp.document.editable, {
    ...
    initialize: function($super, id, name, config, data, inherited) {
        ...
        if (typeof this.config['uploadPath'] === 'undefined') {
            this.config['uploadPath'] = opendxp.settings.asset_default_upload_path;
        }
    ...
    render: function () {
        ...
        this.altBar.addCls("opendxp_editable_image_alt");
        ...
        if(this.config["disableInlineUpload"] !== true) {
            this.element.insertHtml("beforeEnd",'<div class="opendxp_editable_droptarget_upload"></div>');
            this.element.addCls("opendxp_editable_image_empty");
            opendxp.helpers.registerAssetDnDSingleUpload(this.element.dom, this.config["uploadPath"], 'path', function (e) {
            ...
            this.element.insertHtml("beforeEnd",'<div class="opendxp_editable_droptarget"></div>');
            this.element.addCls("opendxp_editable_image_no_upload_empty");
        }
    ...
    onContextMenu: function (e) {
        ...
        if(this.datax.id) {
            ...
            if(this.config['focal_point_context_menu_item']) {
                menu.add(new Ext.menu.Item({
                    ...
                    handler: function (item) {
                        opendxp.helpers.openAsset(this.datax.id, 'image');
                    }.bind(this)
                }));
            }
            ...
            menu.add(new Ext.menu.Item({
                ...
                iconCls: "opendxp_icon_image opendxp_icon_overlay_edit",
                handler: function (item) {
                    item.parentMenu.destroy();
            ...
            if (opendxp.elementservice.showLocateInTreeButton("document")) {
                menu.add(new Ext.menu.Item({
                    ...
                    handler: function (item) {
                        item.parentMenu.destroy();
                        opendxp.treenodelocator.showInTree(this.datax.id, "asset");
                    }.bind(this)
                }));
            }
        }
    ...
    updateImage: function () {
        ...
        if (!this.config["thumbnail"]) {
            if(!this.originalDimensions["width"] && !this.originalDimensions["height"]) {
                path = Routing.generate('opendxp_admin_asset_getimagethumbnail', this.getThumbnailConfig({
        ...
        this.element.removeCls("opendxp_editable_image_empty");
    ...
    openEditWindow: function() {
        ...
        var editor = opendxp.helpers.openImageCropper(this.datax.id, this.datax, function (data) {
    ...
    openHotspotWindow: function() {
        var editor = opendxp.helpers.openImageHotspotMarkerEditor(
            this.datax.id,
            this.datax,
            function (data) {
