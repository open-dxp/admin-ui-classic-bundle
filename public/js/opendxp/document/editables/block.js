 ...
 * @private
 */
opendxp.document.editables.block = Class.create(opendxp.document.editable, {
    ...
    refresh: function() {
        ...
        if (this.elements.length < 1) {
            ...
            Ext.get(this.id).removeCls("opendxp_block_buttons");
    ...
    refreshControls: function (element, limitReached) {
        ...
        amountBox = Ext.get(element).query('.opendxp_block_amount[data-name="' + this.name + '"] .opendxp_block_amount_select', false)[0];
        ...
        if (!limitReached) {
            amountDiv = Ext.get(element).query('.opendxp_block_amount[data-name="' + this.name + '"]')[0];
            if (amountDiv) {
                amountBox = new Ext.form.ComboBox({
                    cls: "opendxp_block_amount_select",
    ...
    render: function () {
        ...
        Ext.get(this.id).on('mouseenter', function (event) {
            Ext.get(this.id).addCls('opendxp_block_entry_over');
        });

        Ext.get(this.id).on('mouseleave', function (event) {
            Ext.get(this.id).removeCls('opendxp_block_entry_over');
        });
    },

    setInherited: function ($super, inherited) {
        var elements = Ext.get(this.id).query('.opendxp_block_buttons[data-name="' + this.name + '"]');
    ...
    createInitalControls: function (amountValues) {
        ...
        amountEl.setAttribute("class", "opendxp_block_amount");
        ...
        Ext.get(this.id).addCls("opendxp_block_limitnotreached opendxp_block_buttons");
    },
    ...
    addBlock : function (element, amountbox, reload = true) {
        ...
        if(this.config['reload'] === true) {
            ...
            editWindow.lastScrollposition = '#' + this.id + ' .opendxp_block_entry[data-name="' + this.name + '"][key="' + firstNewKey + '"]';
            if(reload) {
                this.reloadDocument();
            }
        } else {
            ...
            for (let p = 0; p < amount; p++) {
                elements = Ext.get(this.id).query('.opendxp_block_entry[data-name="' + this.name + '"][key]');
                nextKey++;
                let blockHtml = template;
                blockHtml = blockHtml.replaceAll(new RegExp('"([^"]+):1000000.' + this.getRealName() + '("|:)', 'g'), '"' + this.getName() + '$2');
                blockHtml = blockHtml.replaceAll(new RegExp('"opendxp_editable_([^"]+)_1000000_' + this.getRealName() + '_', 'g'), '"opendxp_editable_' + this.getName().replaceAll(/(:|\.)/g, '_') + '_');
                ...
                this.config['template']['editables'].forEach(editableDef => {
                    ...
                    editable['id'] = editable['id'].replace(new RegExp('opendxp_editable_([^"]+)_1000000_' + this.getRealName() + '_'), 'opendxp_editable_' + this.getName().replaceAll(/(:|\.)/g, '_') + '_');
