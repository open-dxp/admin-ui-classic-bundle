...
opendxp.registerNS("opendxp.document.document");
/**
 * @private
 */
opendxp.document.document = Class.create(opendxp.element.abstract, {
    willClose: false,
    ...
    getData: function () {
        ...
        Ext.Ajax.request({
            ...
            failure: function () {
                opendxp.helpers.forgetOpenTab("document_" + this.id + "_" + this.type);
                opendxp.helpers.closeDocument(this.id);
            }.bind(this)
        });
    },
    ...
    getDataComplete: function (response) {
        try {
            ...
            if (typeof this.data.editlock == "object") {
                opendxp.helpers.lockManager(this.id, "document", this.getType(), this.data);
            ...
            if (this.isAllowed("view")) {
                ...
                if (this.getAddToHistory()) {
                    opendxp.helpers.recordElement(this.id, "document", this.data.path + this.data.key);
                }
    ...
    selectInTree: function () {
        try {
            opendxp.treenodelocator.showInTree(this.id, "document");
        } catch (e) {
            console.log(e);
        }
    },
    ...
    save: function (task, only, callback, successCallback) {
        ...
        if (saveData) {
            ...
            const preSaveDocument = new CustomEvent(opendxp.events.preSaveDocument, {
                detail: {
                    document: this,
                    type: this.getType(),
                    task: task,
                ...
                success: function (response) {
                    try {
                        ...
                        if (rdata && rdata.success) {
                            ...
                            const postSaveDocument = new CustomEvent(opendxp.events.postSaveDocument, {
                                detail: {
                                    document: this,
                                    type: this.getType(),
                                    task: task,
                            ...
                            opendxp.helpers.updateTreeElementStyle('document', this.id, rdata.treeData);
                        }
                    } catch (e) {
    ...
    reload: function () {
        this.tab.on("close", function () {
            ...
            window.setTimeout(function (id, type) {
                opendxp.helpers.openDocument(id, type, {tabIndex: currentTabIndex});
    ...
    linkTranslation: function () {
        ...
        var checkLanguage = function (el) {
            ...
            Ext.Ajax.request({
                url: Routing.generate('opendxp_admin_document_document_translationchecklanguage'),
                params: {
                    path: el.getValue()
                },
                success: function (response) {
                    ...
                    if (data["success"]) {
                        win.getComponent("language").setValue(opendxp.available_languages[data["language"]] + " [" + data["language"] + "]");
        ...
        win = new Ext.Window({
            ...
            buttons: [{
                ...
                handler: function () {
                    ...
                    Ext.Ajax.request({
                        url: Routing.generate('opendxp_admin_document_document_translationadd'),
                        method: 'POST',
                        params: {
                            sourceId: this.id,
    ...
    showDocumentOverview: function () {
        ...
        new opendxp.document.document_language_overview(this);
    },
            ...
            items: [{
                ...
                listeners: {
                    select: function (el) {
                        ...
                        Ext.Ajax.request({
                            url: Routing.generate('opendxp_admin_document_document_translationdetermineparent'),
                            params: {
        ...
        if (this.data["translations"]) {
            ...
            Ext.iterate(this.data["translations"], function (language, documentId, myself) {
                translationsMenu.push({
                    text: opendxp.available_languages[language] + " [" + language + "]",
                    iconCls: "opendxp_icon_language_" + language.toLowerCase(),
                    handler: function () {
                        opendxp.helpers.openElement(documentId, "document");
                    }
                });
            });
        ...
        if (this.data["unlinkTranslations"]) {
            ...
            Ext.iterate(this.data["unlinkTranslations"], function (language, documentId, myself) {
                unlinkTranslationsMenu.push({
                    ...
                    handler: function () {
                        Ext.Ajax.request({
                            url: Routing.generate('opendxp_admin_document_document_translationremove'),
                            method: 'DELETE',
                            params: {
                                sourceId: me.id,
                                targetId: documentId
    ...
    resetPath: function () {
        Ext.Ajax.request({
            url: Routing.generate('opendxp_admin_document_document_getdatabyid'),
            params: {id: this.id},
            success: function (response) {
                var rdata = Ext.decode(response.responseText);
    ...
    getDataRoute: function() {
        return "opendxp_admin_document_" + this.type + '_getdatabyid';
    },

    getSaveRoute: function() {
        return "opendxp_admin_document_" + this.type + '_save';
    },

    getAddRoute: function() {
        return "opendxp_admin_document_document_add";
    }
});
