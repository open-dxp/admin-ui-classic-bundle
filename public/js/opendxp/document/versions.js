...
opendxp.registerNS("opendxp.document.versions");
/**
 * @private
 */
opendxp.document.versions = Class.create({
    ...
    getLayout: function () {
        ...
        if (this.layout == null) {
            ...
            var modelName = 'opendxp.model.documentversions';
    ...
    compareVersions: function (id1, id2) {
        var url = Routing.generate('opendxp_admin_document_document_diffversions', {from: id1, to: id2});;
        Ext.get(this.frameId).dom.src = url;
    },

    showVersionPreview: function (id) {
        var url = this.document.data.path + this.document.data.key + "?opendxp_version=" + id;
        Ext.get(this.frameId).dom.src = url;
    },
    ...
    openVersion: function (index, grid) {
        ...
        window.open(this.document.data.path + this.document.data.key + '?opendxp_version=' + versionId, '_blank');
    },
    ...
    editVersion: function (index, grid) {
        ...
        Ext.Ajax.request({
            url: Routing.generate('opendxp_admin_document_document_versiontosession'),
            method: 'POST',
            params: {id: versionId},
    ...
    publishVersion: function (index, grid) {
        ...
        Ext.Ajax.request({
            url: Routing.generate('opendxp_admin_document_document_publishversion'),
            method: "POST",
            params: {id: versionId},
            success: function(response) {
                ...
                if (rdata.success) {
                    ...
                    opendxp.helpers.updateTreeElementStyle('document', this.document.id, rdata.treeData);
                } else {
