...
opendxp.registerNS("opendxp.document.document_language_overview");
/**
 * @private
 */
opendxp.document.document_language_overview = Class.create({

    initialize: function (document) {

        Ext.Ajax.request({
            url: Routing.generate('opendxp_admin_document_document_languagetreeroot'),
            params: {
                id: document.id
            },
    ...
    getTreeGrid: function (rootNodeConfig, columns, languages) {
        ...
        for(var i=1; i<columns.length; i++) {
            columns[i].renderer = function(value) {
                ...
                var actionsButton = '<img src="/bundles/opendxpadmin/img/flat-color-icons/edit.svg" class="x-action-col-icon x-action-col-0" style="position: absolute;" />';
        ...
        var store = Ext.create('Ext.data.TreeStore', {
            proxy: {
                type: 'ajax',
                url: Routing.generate('opendxp_admin_document_document_languagetree'),
        ...
        if(data.itemType != 'empty') {
            if(data.permissions.view) {
                menu.add([{
                    ...
                    handler: function() {
                        opendxp.helpers.openDocument(data.id, data.type);
                        this.hideWindow();
                    }.bind(this)
                }]);
            }
                                    ...
                                    success: function (response) {
                                        ...
                                        if(resData.success) {
                                            if(params["key"].length >= 1) {
                                                ...
                                                Ext.Ajax.request({
                                                    url: Routing.generate('opendxp_admin_document_document_add'),
                                                    method: 'POST',
                                                    params: params,
                                                    success: function (response) {
