...
opendxp.registerNS("opendxp.asset.helpers.grid");
/**
 * @private
 */
opendxp.asset.helpers.grid = Class.create({
    ...
    getStore: function(noBatchColumns, batchAppendColumns, batchRemoveColumns) {
        ...
        for (var i = 0; i < this.fields.length; i++) {
            if (!in_array(this.fields[i].key, ["creationDate", "modificationDate"])) {
                ...
                let currentTag = opendxp.asset.metadata.tags[type];

                if (currentTag) {
                    if (typeof currentTag.prototype.addGridOptionsFromColumnConfig == "function") {

                        readerFieldConfig["convert"] = opendxp.asset.metadata.tags[type].prototype.addGridOptionsFromColumnConfig.bind(this, key);
                        ...
                        renderer: function (value) {
                            if (value) {
                                return  '<div class="list-img-container">' +
                                    '<img src="' + value + '" loading="lazy" draggable="false">'+
                                    '</div>'
                            }
                        }.bind(this)
                    });
