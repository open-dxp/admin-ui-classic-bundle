...
opendxp.registerNS("opendxp.asset.helpers.gridConfigDialog");
/**
 * @private
 */
opendxp.asset.helpers.gridConfigDialog = Class.create(opendxp.element.helpers.gridConfigDialog, {
    ...
    commitData: function (save, preview) {
        ...
        if (!operatorFound) {
            ...
            Ext.Ajax.request({
                url: Routing.generate('opendxp_admin_asset_assethelper_preparehelpercolumnconfigs'),
                method: 'POST',
                params: {
    ...
    requestPreview: function () {
        ...
        Ext.Ajax.request({
            url: Routing.generate('opendxp_admin_asset_gridproxy'),
            params: {
    ...
    getSelectionPanel: function () {
        if (!this.selectionPanel) {
            ...
            for (let i = 0; i < opendxp.settings.websiteLanguages.length; i++) {
                languagestore.push([opendxp.settings.websiteLanguages[i],
                    opendxp.available_languages[opendxp.settings.websiteLanguages[i]]]);
            }
                    ...
                    renderer: function (value, metaData, record) {
                        if (record && record.parentNode.id == 0) {
                            ...
                            if (key == "preview" && value) {
                                ...
                                try {
                                    if (record.data.isOperator && record.data.configAttributes && opendxp.asset.metadata.tags[record.data.configAttributes.renderer]) {
                                        var rendererType = record.data.configAttributes.renderer;
                                        var tag = opendxp.asset.metadata.tags[rendererType];
                                    } else {
                                        var tag = opendxp.asset.metadata.tags[fieldType];
                                    }
                                ...
                                if (typeof opendxp.asset.metadata.tags[fieldType] !== "undefined" && typeof opendxp.asset.metadata.tags[fieldType].prototype.previewRenderer == "function") {
                                    value = opendxp.asset.metadata.tags[fieldType].prototype.previewRenderer(value, record);
                                }
    ...
    getMetadataTreePanel: function () {
        if (!this.metadataTreePanel) {
            let url = Routing.generate('opendxp_admin_asset_assethelper_getmetadataforcolumnconfig');
    ...
    getMetadataTree: function (url) {
        var metadataTreeHelper = new opendxp.asset.helpers.metadataTree(this.showFieldname);
