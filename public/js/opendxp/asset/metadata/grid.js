...
opendxp.registerNS("opendxp.asset.metadata.grid");
/**
 * @private
 */
opendxp.asset.metadata.grid = Class.create({

    initialize: function (config) {
        this.config = config;
        this.asset = config.asset;

        /** @type {opendxp.asset.metadata.dataProvider} */
    ...
    viewerLayout: function () {
        ...
        this.grid = Ext.create('Ext.grid.Panel', {
            ...
            autoScroll: true,
            region: "center",
            iconCls: this.config.hasOwnProperty('iconCls') ? this.config.iconCls : "opendxp_material_icon_metadata opendxp_material_icon",
                        ...
                        items: [
                            {
                                ...
                                handler: function (grid, rowIndex) {
                                    ...
                                    if (typeof opendxp.asset.metadata.tags[rec.get('type')] !== "undefined") {
                                        opendxp.asset.metadata.tags[rec.get('type')].prototype.handleGridOpenAction(grid, rowIndex);
                                    }
                                }.bind(this),
                                getClass: function (v, meta, rec) {
                                    if (typeof opendxp.asset.metadata.tags[rec.get('type')] !== "undefined") {
                                        return opendxp.asset.metadata.tags[rec.get('type')].prototype.getGridOpenActionVisibilityStyle();
                                    }
                                }
                            }
                        ]
    ...
    editorLayout: function () {
        ...
        const supportedTypes = opendxp.helpers.getAssetMetadataDataTypes("custom");
        const typeStore = [];
        ...
        const websiteLanguages = opendxp.settings.websiteLanguages;
        let selectContent = "";
        ...
        const modelName = 'opendxp.model.assetmetadata';
                    ...
                    fields: [
                        {
                            ...
                            convert: function (v, r) {
                                ...
                                if (typeof opendxp.asset.metadata.tags[dataType] !== "undefined") {
                                    if (typeof opendxp.asset.metadata.tags[dataType].prototype.convertPredefinedGridData === "function") {
                                        v = opendxp.asset.metadata.tags[dataType].prototype.convertPredefinedGridData(v, r);
                                    }
                                }
                                return v;
                            }
        ...
        const store = new Ext.data.Store({
            ...
            listeners: {
                update: function (store, record, operation, modifiedFieldNames, details, eOpts) {
                    ...
                    if (typeof opendxp.asset.metadata.tags[record.data.type] !== "undefined") {
                        newData = opendxp.asset.metadata.tags[record.data.type].prototype.marshal(newData);
                    }
        ...
        for (let i = 0; i < rows.length; i++) {
            try {
                ...
                if (in_array(data.name, this.disallowedKeys)) {
                    Ext.get(rows[i]).addCls("opendxp_properties_hidden_row");
                }

                if (typeof opendxp.asset.metadata.tags[data.type] !== "undefined") {
                    opendxp.asset.metadata.tags[data.type].prototype.updatePredefinedGridRow(this.grid, rows[i], data);
                }
            } catch (e) {
                console.log(e);
            }
    ...
    getTypeRenderer: function (value, metaData, record, rowIndex, colIndex, store) {
        return '<div class="opendxp_icon_' + Ext.util.Format.htmlEncode(value) + ' opendxp_property_grid_type_column" name="' + Ext.util.Format.htmlEncode(record.data.name) + '">&nbsp;</div>';
    },

    getCellRenderer: function (value, metaData, record, rowIndex, colIndex, store) {
        var data = store.getAt(rowIndex).data;
        var type = data.type;
        if (typeof opendxp.asset.metadata.tags[type] == "undefined") {
            type = "input";
        }
        return opendxp.asset.metadata.tags[type].prototype.getGridCellRenderer(value, metaData, record, rowIndex, colIndex, store);
    },
    ...
    cellMousedown: function (grid, cell, rowIndex, cellIndex, e) {
        ...
        opendxp.asset.metadata.tags[type].prototype.handleGridCellClick(grid, cell, rowIndex, cellIndex, e);
    },
    ...
    getCellEditor: function (record) {
        ...
        return opendxp.asset.metadata.tags[type].prototype.getGridCellEditor("custom", record);
    },
    ...
    handleAddPredefinedDefinitions: function (group) {
        Ext.Ajax.request({
            url: Routing.generate('opendxp_admin_settings_getpredefinedmetadata'),
            params: {
                type: "asset",
    ...
    doAddPredefinedDefinitions: function (response) {
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            if (duplicateIndex < 0) {
                let value = item.data;
                if (typeof opendxp.asset.metadata.tags[item.type] !== "undefined") {
                    value = opendxp.asset.metadata.tags[item.type].prototype.unmarshal(value);
                }
