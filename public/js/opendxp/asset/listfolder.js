...
opendxp.registerNS("opendxp.asset.listfolder");
/**
 * @private
 */
opendxp.asset.listfolder = Class.create(opendxp.asset.helpers.gridTabAbstract, {
    ...
    getLayout: function () {
        if (this.layout == null) {
            ...
            this.layout = new Ext.Panel({
                title: t("list"),
                iconCls: "opendxp_material_icon_list opendxp_material_icon",
                border: false,
                layout: "border"
            });
    ...
    createGrid: function (fromConfig, response, settings, save, context) {
        ...
        var gridHelper = new opendxp.asset.helpers.grid(
            fields,
            Routing.generate('opendxp_admin_asset_gridproxy'),
            {
        ...
        this.grid = Ext.create('Ext.grid.Panel', {
            ...
            plugins: [this.cellEditing, 'opendxp.gridfilters'],
            trackMouseOver: true,
        ...
        if (selectedRows.length <= 1) {
            ...
            menu.add(new Ext.menu.Item({
                ...
                handler: function (data) {
                    opendxp.helpers.openAsset(data.id, data.data['type~system']);
                }.bind(this, data)
            }));
            ...
            if (opendxp.elementservice.showLocateInTreeButton("asset")) {
                menu.add(new Ext.menu.Item({
                    ...
                    handler: function () {
                        try {
                            try {
                                opendxp.treenodelocator.showInTree(record.id, "asset", this);
                            } catch (e) {
                                console.log(e);
                            }
...
opendxp.asset.listfolder.addMethods(opendxp.element.helpers.gridColumnConfig);
