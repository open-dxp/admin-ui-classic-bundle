...
opendxp.registerNS("opendxp.settings.profile.panel");
/**
 * @private
 */
opendxp.settings.profile.panel = Class.create({
    ...
    getTabPanel: function () {
        ...
        if (!this.panel) {
            ...
            this.panel.on("destroy", function () {
                opendxp.globalmanager.remove("profile");
            }.bind(this));
    ...
    getEditPanel: function () {
        ...
        this.currentUser = opendxp.currentuser;

        var passwordCheck = function (el) {
            if (opendxp.helpers.isValidPassword(el.getValue())) {
        ...
        baseItems.push({
            ...
            name: "language",
            listWidth: 100,
            store: opendxp.globalmanager.get("opendxplanguages"),
                ...
                items: [
                    ...
                    passwordField,
                    {
                        ...
                        handler: function () {
                            ...
                            while (true) {
                                pass = opendxp.helpers.generatePassword(15);
                                if (opendxp.helpers.isValidPassword(pass)) {
                                    break;
                                }
                            }
        ...
        var twoFactorSettings = new opendxp.settings.profile.twoFactorSettings(this.currentUser.twoFactorAuthentication);
            ...
            items: [
                {
                    ...
                    items: [{
                        xtype: "image",
                        id: "opendxp_profile_image_" + this.currentUser.id,
                        src: Routing.generate('opendxp_admin_user_getimage', {id: this.currentUser.id, '_dc': Ext.Date.now()}),
                        width: 45,
                        height: 45
                    }],
                ...
                },
                {
                    ...
                    handler: function () {
                        opendxp.helpers.uploadDialog(
                            Routing.generate('opendxp_admin_user_uploadcurrentuserimage', {id: this.currentUser.id}),
                            null,
                            function () {
                                Ext.getCmp("opendxp_profile_delete_image_" + this.currentUser.id).setVisible(true);
                                opendxp.helpers.reloadUserImage(this.currentUser.id);
                ...
                },
                {
                    ...
                    tooltip: t("remove"),
                    id: "opendxp_profile_delete_image_" + this.currentUser.id,
                    hidden: !this.currentUser.hasImage,
                    handler: function () {
                        Ext.Ajax.request({
                            url: Routing.generate('opendxp_admin_user_deleteimage', {id: this.currentUser.id}),
                            method: 'DELETE',
                            success: function() {
                                Ext.getCmp("opendxp_profile_delete_image_" + this.currentUser.id).setVisible(false);
                                opendxp.helpers.reloadUserImage(this.currentUser.id);
        ...
        this.editorSettings = new opendxp.settings.user.editorSettings(this, this.currentUser.contentLanguages);
        ...
        this.keyBindings = new opendxp.settings.user.user.keyBindings(this, true);
    ...
    saveCurrentUser: function () {
        ...
        if (values["new_password"]) {
            if (!opendxp.helpers.isValidPassword(values["new_password"])) {
        ...
        if (canBeSaved) {
            Ext.Ajax.request({
                url: Routing.generate('opendxp_admin_user_updatecurrentuser'),
                method: "PUT",
                params: {
                    ...
                    try {
                        ...
                        if (res.success) {
                            ...
                            if (contentLanguages) {
                                ...
                                opendxp.currentuser.contentLanguages = contentLanguages.join(',');
                            }
                        } else {
    ...
    activate: function () {
        Ext.getCmp("opendxp_panel_tabs").setActiveItem("my_profile");
    }
});
