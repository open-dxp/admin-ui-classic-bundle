...
opendxp.registerNS("opendxp.settings.videothumbnail.panel");
/**
 * @private
 */
opendxp.settings.videothumbnail.panel = Class.create({
    ...
    activate: function () {
        ...
        tabPanel.setActiveItem("opendxp_videothumbnails");
    },
    ...
    getTabPanel: function () {
        ...
        if (!this.panel) {
            ...
            this.panel.on("destroy", function () {
                opendxp.globalmanager.remove("videothumbnails");
            }.bind(this));
    ...
    getTree: function () {
        if (!this.tree) {
            var store = Ext.create('Ext.data.TreeStore', {
                ...
                proxy: {
                    type: 'ajax',
                    url: Routing.generate('opendxp_admin_settings_videothumbnailtree'),
                    reader: {
                        type: 'json'
                    }
                },
            ...
            this.tree = new Ext.tree.TreePanel({
                store: store,
                id: "opendxp_panel_videothumbnail_tree",
                region: "west",
                autoScroll:true,
                animate:false,
                    ...
                    items: [
                        {
                            ...
                            disabled: !opendxp.settings['video-thumbnails-writeable']
                        }
                    ]
                }
            });
    ...
    openThumbnail: function (id) {
        var existingPanel = Ext.getCmp("opendxp_videothumbnail_panel_" + id);
        if(existingPanel) {
        ...
        Ext.Ajax.request({
            url: Routing.generate('opendxp_admin_settings_videothumbnailget'),
            params: {
                name: id
            },
            success: function (response) {
                ...
                var fieldPanel = new opendxp.settings.videothumbnail.item(data, this);
                opendxp.layout.refresh();
            }.bind(this)
        });
    },
    ...
    addFieldComplete: function (button, value, object) {
        ...
        if (button == "ok" && value.length > 2 && regresult == value) {
            ...
            Ext.Ajax.request({
                url: Routing.generate('opendxp_admin_settings_videothumbnailadd'),
                method: "POST",
                params: {
                    name: value
                },
    ...
    deleteField: function (tree, record) {
        opendxp.helpers.deleteConfirm(t('thumbnail'), record.data.text, function () {
            Ext.Ajax.request({
                url: Routing.generate('opendxp_admin_settings_videothumbnaildelete'),
                method: 'DELETE',
                params: {
                    name: record.data.id
                }
