...
opendxp.registerNS("opendxp.settings.user.user.keyBindings");
/**
 * @private
 */
opendxp.settings.user.user.keyBindings = Class.create({
    ...
    buildItems: function(userBindings) {
        var mapping = opendxp.helpers.keyBindingMapping;
        ...
        for (var action in mapping) {
            if (mapping.hasOwnProperty(action)) {
                ...
                generalItems.push(new Ext.form.field.Text({
                    ...
                    listeners: {
                        "focus": function () {
                            opendxp.keymap.disable();
                            window.document.onkeydown = function () {
                            };
                        },
                        "blur": function () {
                            window.document.onkeydown = null;
                            opendxp.keymap.enable();
                        },
    ...
    getPanel: function (responseData) {
        ...
        this.panel.add({
            ...
            handler: function() {
                Ext.Ajax.request({
                    url: Routing.generate('opendxp_admin_user_getdefaultkeybindings'),
        ...
        this.panel.add({
            xtype: 'panel',
            html: t('please_dont_forget_to_reload_opendxp'),
            minHeight:50
        });
