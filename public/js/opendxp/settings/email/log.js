...
opendxp.registerNS('opendxp.settings.email.log');
/**
 * @private
 */
opendxp.settings.email.log = Class.create({
    ...
    initialize: function(document) {
        ...
        if(!this.document) {
            ...
            this.getLayout().on('destroy', function () {
                opendxp.globalmanager.remove('sent_emails');
            }.bind(this));
        }
    },
    ...
    getLayout: function () {
        ...
        if (this.layout == null) {
            ...
            this.layout = new Ext.Panel({
                ...
                iconCls: this.document ? 'opendxp_material_icon_email_sent opendxp_material_icon' : 'opendxp_icon_email opendxp_icon_overlay_go',
                listeners: {
                    activate: function() {
                        this.store.load();
    ...
    getGrid: function () {
        ...
        var gridColumns = [
            ...
            },
            {
                ...
                text: t('html'),
                items : [{
                    icon: '/bundles/opendxpadmin/img/flat-color-icons/feedback.svg',
                    handler: function(grid, rowIndex){
                        var rec = grid.getStore().getAt(rowIndex);
                        var url = Routing.generate('opendxp_admin_email_showemaillog', {id: rec.get('id'), type: 'html'});
            ...
            },
            {
                ...
                text: t('text'),
                items : [{
                    icon: '/bundles/opendxpadmin/img/flat-color-icons/text.svg',
                    handler: function(grid, rowIndex){
                        var rec = grid.getStore().getAt(rowIndex);
                        var url = Routing.generate('opendxp_admin_email_showemaillog', {id: rec.get('id'), type: 'text'});
                        var iframe = new Ext.Window({
                            title: t('text'),
                            width: iFrameSettings.width,
                            height: iFrameSettings.height,
            ...
            },
            {
                ...
                text: t('parameters'),
                items : [{
                    icon: '/bundles/opendxpadmin/img/flat-color-icons/info.svg',
                    handler: function(grid, rowIndex){
                        var rec = grid.getStore().getAt(rowIndex);
                        var url = Routing.generate('opendxp_admin_email_showemaillog', {id: rec.get('id'), type: 'params'});
            ...
            },
            {
                ...
                text: t('error'),
                items: [{
                    icon: '/bundles/opendxpadmin/img/flat-color-icons/high_priority.svg',
                    handler: function (grid, rowIndex) {
                        var rec = grid.getStore().getAt(rowIndex);
                        var iframe = new Ext.Window({
                            title: t('text'),
                ...
                items:[
                    {
                        ...
                        icon: '/bundles/opendxpadmin/img/flat-color-icons/email-forward.svg',
                        handler: function (grid, rowIndex) {
                            var rec = grid.getStore().getAt(rowIndex);
                ...
                items: [{
                    ...
                    handler: function (grid, rowIndex) {
                        ...
                        opendxp.helpers.deleteConfirm(t('email_log'), data.id, function () {
                            var rec = grid.getStore().getAt(rowIndex);
                            Ext.Ajax.request({
                                url: Routing.generate('opendxp_admin_email_deleteemaillog'),
                                method: 'DELETE',
                                success: function(response){
                                    var data = Ext.decode( response.responseText );
                                    if(!data.success){
    ...
    getForwardEmailWindow: function (data) {
        if (data) {
            ...
            var emailPreviewUrl = Routing.generate('opendxp_admin_email_showemaillog', {id: data.id, type: emailType});
