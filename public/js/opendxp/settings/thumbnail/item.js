...
opendxp.registerNS("opendxp.settings.thumbnail.item");
/**
 * @private
 */
opendxp.settings.thumbnail.item = Class.create({
    ...
    addLayout: function () {
        ...
        this.settings = new Ext.form.FormPanel({
            ...
            items: [{
                ...
                border: false,
                loader: {
                    url: Routing.generate('opendxp_admin_settings_thumbnailadaptercheck'),
                    autoLoad: true
                }
            }, {
                xtype: "textfield",
        ...
        this.panel = new Ext.Panel({
            ...
            id: "opendxp_thumbnail_panel_" + this.data.name,
    ...
    addMediaPanel: function (name, items, closable, activate) {
        ...
        var addMenu = [];
        var itemTypes = Object.keys(opendxp.settings.thumbnail.items);
        for (var i = 0; i < itemTypes.length; i++) {
            if (itemTypes[i].indexOf("item") == 0) {
                addMenu.push({
                    iconCls: "opendxp_icon_add",
                    handler: this.addItem.bind(this, name, itemTypes[i]),
                    text: opendxp.settings.thumbnail.items[itemTypes[i]](null, null, true)
                });
            }
        }
    ...
    addItem: function (name, type, data) {
        ...
        var item = opendxp.settings.thumbnail.items[type](this.medias[name], data);
    ...
    save: function () {
        Ext.Ajax.request({
            url: Routing.generate('opendxp_admin_settings_thumbnailupdate'),
 ...
 * ITEM TYPES
 */
opendxp.registerNS("opendxp.settings.thumbnail.items");
/**
 * @private
 */
opendxp.settings.thumbnail.items = {
