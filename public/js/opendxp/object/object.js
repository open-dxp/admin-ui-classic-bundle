...
opendxp.registerNS("opendxp.object.object");
/**
 * @private
 */
opendxp.object.object = Class.create(opendxp.object.abstract, {
    ...
    initialize: function (id, options) {
        ...
        const preOpenObject = new CustomEvent(opendxp.events.preOpenObject, {
            detail: {
                object: this,
                type: "object"
            },
        ...
        this.edit = new opendxp.object.edit(this);
        this.preview = new opendxp.object.preview(this);
        this.properties = new opendxp.element.properties(this, "object");
        this.versions = new opendxp.object.versions(this);
        this.scheduler = new opendxp.element.scheduler(this, "object");
        ...
        if(opendxp.globalmanager.get('customReportsPanelImplementationFactory').hasImplementation()) {
            this.reports = opendxp.globalmanager.get('customReportsPanelImplementationFactory').getNewReportInstance("object_concrete");
        }
        this.variants = new opendxp.object.variantsTab(this);
        if(opendxp.globalmanager.get('applicationLoggerPanelImplementationFactory').hasImplementation()) {
            this.appLogger = opendxp.globalmanager.get('applicationLoggerPanelImplementationFactory').getNewLoggerInstance({localMode: true,
                searchParams: {
        ...
        this.frontendLanguages = opendxp.settings.websiteLanguages;
    ...
    getDataComplete: function (response) {
        try {
            ...
            if (typeof this.data.editlock == "object") {
                opendxp.helpers.lockManager(this.id, "object", "object", this.data);
            ...
            opendxp.elementservice.setElementPublishedState({
                elementType: "object",
                id: this.id,
            ...
            if (opendxp.globalmanager.get('global_language_' + this.id)) {
                opendxp.globalmanager.remove('global_language_' + this.id);
            }
    ...
    addTab: function () {
        ...
        if (this.data.general["iconCls"]) {
            ...
            iconClass = opendxp.helpers.getClassForIcon(this.data.general["icon"]);
        }
        ...
        if (opendxp.helpers.checkIfNewHeadbarLayoutIsEnabled()) {
            this.tab = new Ext.Panel({
                ...
                object: this,
                cls: "opendxp_panel_toolbar_horizontal_border_layout opendxp_class_" + this.data.general.className,
                iconCls: iconClass
            });
            ...
            this.tab = new Ext.Panel({
                ...
                object: this,
                cls: "opendxp_class_" + this.data.general.className,
                iconCls: iconClass
            });
        }
        ...
        if (this.getAddToHistory()) {
            opendxp.helpers.recordElement(this.id, "object", this.data.general.fullpath);
        }
    ...
    forgetOpenTab: function () {
        ...
        opendxp.helpers.forgetOpenTab("object_" + this.id + "_object");
        opendxp.helpers.forgetOpenTab("object_" + this.id + "_variant");
    ...
    getTabPanel: function () {
        ...
        if (this.data.childdata.data.classes.length > 0) {
            try {
                this.search = new opendxp.object.search(this.data.childdata, "children");
    ...
    getLayoutToolbar: function () {
        ...
        if (!this.toolbar) {
            ...
            metaInfoMenu.unshift({
                ...
                handler: opendxp.helpers.copyStringToClipboard.bind(this, t(this.data.general.classTitle))
            });
            ...
            if (this.data.general.showFieldLookup) {
                const showLockupConfig = {
                    ...(() => opendxp.helpers.checkIfNewHeadbarLayoutIsEnabled() ? { text: t('fieldlookup') } : { tooltip: t('fieldlookup') })(),
                    iconCls: "opendxp_material_fieldlookup opendxp_material_icon",
                    scale: "medium",
                    handler: function() {
                        ...
                        var dialog = new opendxp.object.fieldlookup.filterdialog(config, null, object);
                        dialog.show();
                    }.bind(this)
                }

                if (opendxp.helpers.checkIfNewHeadbarLayoutIsEnabled()) {
                    this.toolbarSubmenu.menu.add(showLockupConfig);
                } else {
            ...
            if (this.data.hasPreview) {
                const previewConfig = {
                    ...(() => opendxp.helpers.checkIfNewHeadbarLayoutIsEnabled() ? { text: t('open') } : { tooltip: t('open') })(),
                    iconCls: "opendxp_material_icon_preview opendxp_material_icon",
                    scale: "medium",
                    handler: function () {
                        var date = new Date();
                        var path = Routing.generate('opendxp_admin_dataobject_dataobject_preview', {id: this.data.general.id, time: date.getTime()});
                ...
                if (opendxp.helpers.checkIfNewHeadbarLayoutIsEnabled()) {
                    this.toolbarSubmenu.menu.add(previewConfig);
                } else {
                    buttons.push("-");
            ...
            this.languageSwitcher = Ext.create('Ext.button.Split', {
                iconCls: "opendxp_icon_language_" + this.frontendLanguages[0].toLowerCase(),
                cls: "opendxp_translation_button",
                scale: "medium",
                menu: this.getLanguageMenuItems(),
                handler: function() {
                    if (opendxp.globalmanager.get('global_language_' + this.id)) {
                        this.toolbar.fireEvent(
                            opendxp.events.globalLanguageChanged,
                            opendxp.globalmanager.get('global_language_' + this.id)
                        );
                    }
                }.bind(this)
            });
    ...
    save: function (task, only, callback, successCallback) {
        ...
        if (this.saveInProgress()){
            opendxp.helpers.showNotification(t("warning"), t("Another saving process is in progress, please wait and retry again"), "info");
            return;
        }
                        ...
                        try {
                            ...
                            if (rdata) {
                                if (rdata.success) {
                                    ...
                                    opendxp.helpers.updateTreeElementStyle('object', this.id, rdata.treeData);
                                    ...
                                    opendxp.helpers.showPrettyError("error", t("saving_failed"), rdata.message);
                                }
                            }
                        } catch (e) {
    ...
    reload: function (params) {
        ...
        this.tab.on("close", function () {
            ...
            window.setTimeout(function (id) {
                opendxp.helpers.openObject(id, "object", options);
    ...
    getMetaInfo: function() {
        return {
            ...
            deeplink: opendxp.helpers.getDeeplink("object", this.data.general.id, this.data.general.type)
        };
    },
