...
opendxp.registerNS("opendxp.object.classificationstore.groupsPanel");
/**
 * @private
 */
opendxp.object.classificationstore.groupsPanel = Class.create({
    ...
    getRelationsProxy: function() {
        let proxy = {
            url: Routing.generate('opendxp_admin_dataobject_classificationstore_relationsactionget'),
            type: 'ajax',
            batchActions: false,
            reader: {
                type: 'json',
    ...
    createRelationsGrid: function() {
            ...
            items: [
                {
                    ...
                    handler: function (grid, rowIndex) {
                        ...
                        Ext.Msg.confirm(t('delete'), sprintf(t('delete_message_advanced'), t('classificationstore_relation'), Ext.util.Format.htmlEncode(data.data.keyName)), function(btn) {
                            if (btn == 'yes') {
                                Ext.Ajax.request({
                                    url: Routing.generate('opendxp_admin_dataobject_classificationstore_deleterelation'),
                                    method: 'DELETE',
                                    params: {
                                        keyId: keyId,
                                        groupId: groupId
        ...
        this.relationsPagingtoolbar = opendxp.helpers.grid.buildDefaultPagingToolbar(this.relationsStore, {pageSize: pageSize});
    ...
    createGroupsGrid: function(response) {
        ...
        var proxy = {
            url: Routing.generate('opendxp_admin_dataobject_classificationstore_groupsactionget'),
            type: 'ajax',
            batchActions: false,
            reader: {
                type: 'json',
            ...
            items: [
                {
                    ...
                    handler: function (grid, rowIndex) {
                        ...
                        Ext.Msg.confirm(t('delete'), sprintf(t('delete_message_advanced') + `</br>${t('delete_data_from_it')}`, t('classificationstore_group'), Ext.util.Format.htmlEncode(data.data.name)), function(btn) {
                            if (btn == 'yes') {
                                ...
                                Ext.Ajax.request({
                                    url: Routing.generate('opendxp_admin_dataobject_classificationstore_deletegroup'),
                                    method: 'DELETE',
                                    params: {
                                        id: id
                                    },
        ...
        var pageSize = opendxp.helpers.grid.getDefaultPageSize(-1);
        this.groupsPagingtoolbar = opendxp.helpers.grid.buildDefaultPagingToolbar(this.groupsStore, {pageSize: pageSize});
    ...
    onAddKey: function() {
        var keySelectionWindow = new opendxp.object.classificationstore.keySelectionWindow({
            parent: this,
            enableKeys: true,
            storeId: this.storeConfig.id
        });
    ...
    addFieldComplete: function (button, value, object) {
        ...
        if (button == "ok" && value.length > 1) {
            Ext.Ajax.request({
                url: Routing.generate('opendxp_admin_dataobject_classificationstore_creategroup'),
                method: 'POST',
                params: {
                    name: value,
                    storeId: this.storeConfig.id
    ...
    openConfig: function(id) {
        ...
        Ext.Ajax.request({
            url: Routing.generate('opendxp_admin_dataobject_classificationstore_getpage'),
            params: params,
            success: function(response) {
                try {
