...
opendxp.registerNS("opendxp.object.classificationstore.collectionsPanel");
/**
 * @private
 */
opendxp.object.classificationstore.collectionsPanel = Class.create({
    ...
    createRelationsGrid: function() {
        ...
        var route = 'opendxp_admin_dataobject_classificationstore_collectionrelations';
        var proxy = {
            batchActions: false,
            type: 'ajax',
            reader: {
            ...
            items: [
                {
                    ...
                    handler: function (grid, rowIndex) {
                        ...
                        Ext.Msg.confirm(t('delete'), sprintf(t('delete_message_advanced'), t('classificationstore_collection_relation'), Ext.util.Format.htmlEncode(data.data.groupName)), function(btn) {
                            if (btn == 'yes') {
                                Ext.Ajax.request({
                                    url: Routing.generate('opendxp_admin_dataobject_classificationstore_deletecollectionrelation'),
                                    method: 'DELETE',
                                    params: {
                                        colId: colId,
                                        groupId: groupId
    ...
    createCollectionsGrid: function(response) {
        ...
        var proxy = {
            url: Routing.generate('opendxp_admin_dataobject_classificationstore_collectionsactionget'),
            batchActions: false,
            type: 'ajax',
            reader: {
                type: 'json',
            ...
            items: [
                {
                    ...
                    handler: function (grid, rowIndex) {
                        ...
                        Ext.Msg.confirm(t('delete'), sprintf(t('delete_message_advanced'), t('classificationstore_collection'), Ext.util.Format.htmlEncode(data.data.name)), function(btn) {
                            if (btn == 'yes') {
                                Ext.Ajax.request({
                                    url: Routing.generate('opendxp_admin_dataobject_classificationstore_deletecollection'),
                                    method: 'DELETE',
                                    params: {
                                        id: id
                                    },
        ...
        var pageSize = opendxp.helpers.grid.getDefaultPageSize(-1);
        this.collectionsPagingtoolbar = opendxp.helpers.grid.buildDefaultPagingToolbar(this.collectionsStore, {pageSize: pageSize});
    ...
    addFieldComplete: function (button, value, object) {
        ...
        if (button == "ok" && value.length > 1) {
            Ext.Ajax.request({
                url: Routing.generate('opendxp_admin_dataobject_classificationstore_createcollection'),
                method: 'POST',
                params: {
                    name: value,
                    storeId: this.storeConfig.id
