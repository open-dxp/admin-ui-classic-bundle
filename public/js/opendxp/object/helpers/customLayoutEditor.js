...
opendxp.registerNS("opendxp.object.helpers.customLayoutEditor");
/**
 * @private
 */
opendxp.object.helpers.customLayoutEditor = Class.create({

    uploadRoute: 'opendxp_admin_dataobject_class_importcustomlayoutdefinition',
    exportRoute: 'opendxp_admin_dataobject_class_exportcustomlayoutdefinition',
    ...
    initialize: function (klass) {
        ...
        this.exportButton = new Ext.Button(
            {
                text: t("export"),
                iconCls: "opendxp_icon_class opendxp_icon_overlay_download",
                disabled: true,
                handler: function() {
        ...
        this.importButton = new Ext.Button(
            {
                text: t("import"),
                iconCls: "opendxp_icon_class opendxp_icon_overlay_upload",
                disabled: true,
    ...
    getLayoutSelection: function () {
        ...
        this.layoutComboStore = new Ext.data.Store({
            proxy: {
                type: 'ajax',
                url: Routing.generate('opendxp_admin_dataobject_class_getcustomlayoutdefinitions'),
                extraParams: {
        ...
        this.layoutChangeCombo = new Ext.form.ComboBox({
            ...
            listeners: {
                ...
                select: function(field, fieldname) {
                    ...
                    Ext.Ajax.request({
                        url: Routing.generate('opendxp_admin_dataobject_class_getcustomlayout'),
                        params: {
                            id: layoutId
                        },
            ...
            items: [this.layoutChangeCombo,
                {
                    ...
                    disabled: !opendxp.settings['object-custom-layout-writeable']
                },
                {
                    xtype: "button",
    ...
    onTreeNodeContextmenu: function (tree, record, item, index, e, eOpts ) {
        ...
        var dataMenu = [];
        dataMenu.push({
            text: opendxp.object.classes.data.localizedfields.prototype.getTypeName(),
            iconCls: opendxp.object.classes.data.localizedfields.prototype.getIconClass(),
    ...
    getClassDefinitionPanel: function () {
        if (!this.classDefinitionPanel) {
            if (typeof this.klass.id !== 'undefined') {
                this.classDefinitionPanel = this.getClassTree(Routing.generate('opendxp_admin_dataobject_class_get'), this.klass.id);
            } else if(typeof this.klass.key !== 'undefined') {
                this.classDefinitionPanel = this.getClassTree(Routing.generate('opendxp_admin_dataobject_class_objectbrickget'), this.klass.key);
            }
        }
    ...
    suggestIdentifier: function() {
        Ext.Ajax.request({
            url: Routing.generate('opendxp_admin_dataobject_class_suggestcustomlayoutidentifier'),
            params: {
                classId: this.klass.id,
            },
            success: function (response) {
    ...
    addLayoutChild: function (record, type, initData, addListener) {
        ...
        newNode.data.editor = new opendxp.object.classes.layout[type](newNode, initData);
    ...
    addDataChild: function (record, type, initData, attributePrefix, showFieldname, addListener) {
        ...
        newNode = record.appendChild(newNode);
        newNode.data.editor = new opendxp.object.classes.data[type](newNode, initData);
    ...
    save: function () {
        ...
        if (this.data["name"].length > 2 && this.data["name"].length < 64 && regresult == this.data["name"]) {
            ...
            if (this.getDataSuccess) {
                Ext.Ajax.request({
                    url: Routing.generate('opendxp_admin_dataobject_class_savecustomlayout'),
                    method: "PUT",
                    params: {
                        configuration: m,
                        values: n,
    ...
    addLayoutComplete: function (layoutName, layoutIdentifier, layouts) {
        ...
        Ext.Ajax.request({
            url: Routing.generate('opendxp_admin_dataobject_class_addcustomlayout'),
            method: 'POST',
            params: {
    ...
    deleteLayout: function () {
        ...
        Ext.Msg.confirm(t('delete'), t('delete_message'), function(btn){
            if (btn == 'yes') {
                Ext.Ajax.request({
                    url: Routing.generate('opendxp_admin_dataobject_class_deletecustomlayout'),
                    method: 'DELETE',
                    params: {
                        id: id
                    }
