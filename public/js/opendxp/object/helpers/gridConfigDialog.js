 ...
 * @private
 */
opendxp.object.helpers.gridConfigDialog = Class.create(opendxp.element.helpers.gridConfigDialog, {
    ...
    commitData: function (save, preview) {
        ...
        if (!operatorFound) {
            ...
            Ext.Ajax.request({
                url: Routing.generate('opendxp_admin_dataobject_dataobjecthelper_preparehelpercolumnconfigs'),
                method: 'POST',
                params: {
                    ...
                    renderer: function (value, metaData, record) {
                        ...
                        if (record && record.parentNode.id == 0) {
                            ...
                            if (key == "modificationDate" || key == "creationDate") {
                                ...
                                try {
                                    if (record.data.isOperator && record.data.configAttributes && opendxp.object.tags[record.data.configAttributes.renderer]) {
                                        var rendererType = record.data.configAttributes.renderer;
                                        var tag = opendxp.object.tags[rendererType];
                                    } else {
    ...
    getClassDefinitionTreePanel: function () {
        if (!this.classDefinitionTreePanel) {
            this.brickKeys = [];
            this.classDefinitionTreePanel = this.getClassTree(Routing.generate('opendxp_admin_dataobject_class_getclassdefinitionforcolumnconfig'),
    ...
    getConfigElement: function (configAttributes) {
        ...
        if (configAttributes && configAttributes.class && configAttributes.type) {
            ...
            if (opendxp.object.gridcolumn[configAttributes.type] && opendxp.object.gridcolumn[configAttributes.type][jsClass]) {
                element = new opendxp.object.gridcolumn[configAttributes.type][jsClass](this.config.classid);
            }
        } else {
            var dataType = configAttributes.dataType.toLowerCase();
            if (opendxp.object.gridcolumn.value[dataType]) {
                element = new opendxp.object.gridcolumn.value[dataType](this.config.classid);
            } else {
                element = new opendxp.object.gridcolumn.value.defaultvalue(this.config.classid);
            }
        }
        return element;
    }
