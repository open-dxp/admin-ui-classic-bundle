...
opendxp.registerNS("opendxp.object.helpers.grid");
/**
 * @private
 */
opendxp.object.helpers.grid = Class.create({
        ...
        for (var i = 0; i < this.fields.length; i++) {
            if (!in_array(this.fields[i].key, ["creationDate", "modificationDate"])) {
                ...
                if (
                    ...
                    if (opendxp.object.tags[type] && opendxp.object.tags[type].prototype.allowBatchAppend) {
                        batchAppendColumns.push(key);
                    }
                    if (opendxp.object.tags[type] && opendxp.object.tags[type].prototype.allowBatchRemove) {
            ...
            if(field.key == "subtype") {
                var columnConfig = {
                    ...
                    renderer: function (value, metaData, record, rowIndex, colIndex, store) {
                        return '<div style="height: 16px;" class="opendxp_icon_asset  opendxp_icon_'
                ...
                if (fields[i].isOperator) {
                    ...
                    if (field.attributes.renderer && opendxp.object.tags[field.attributes.renderer]) {
                        var tag = new opendxp.object.tags[field.attributes.renderer]({}, {});
                    ...
                    var tag = new opendxp.object.tags[fieldType](null, fields[i].layout);
                    if (tag) {
