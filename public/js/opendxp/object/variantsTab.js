...
opendxp.registerNS("opendxp.object.variantsTab");
/**
 * @private
 */
opendxp.object.variantsTab = Class.create(opendxp.object.helpers.gridTabAbstract, {
    ...
    getLayout: function () {
        ...
        if (this.layout == null) {
            ...
            this.layout = new Ext.Panel({
                title: t('variants'),
                border: false,
                iconCls: "opendxp_material_icon_variants opendxp_material_icon",
                layout: "fit"
            });
        }
    ...
    createGrid: function (fromConfig, response, settings, save, context) {
        ...
        var gridHelper = new opendxp.object.helpers.grid(
            this.selectedClass,
            fields,
            Routing.generate('opendxp_admin_dataobject_variants_getvariants'),
            baseParams,
            false
        );
            ...
            items: [
                {
                    ...
                    handler: function (grid, rowIndex) {
                        ...
                        opendxp.helpers.openObject(data.id, "variant");
                    }.bind(this)
                }
            ]
        });
        ...
        let plugins = [this.cellEditing, 'opendxp.gridfilters'];
        ...
        this.grid = Ext.create('Ext.grid.Panel', {
            ...
            listeners: {
                rowdblclick: function (grid, record, tr, rowIndex, e, eOpts) {
                    ...
                    opendxp.helpers.openElement(record.get('id'), record.get('type'), subtype);
                }.bind(this)
            }
        });
    ...
    editKey: function (id, button, value) {
        if (button == "ok") {
            Ext.Ajax.request({
                url: Routing.generate('opendxp_admin_dataobject_variants_updatekey'),
                method: 'PUT',
        ...
        if (button == "ok") {
            Ext.Ajax.request({
                ...
                success: function (response) {
                    ...
                    if (responseJson.success) {
                        this.store.reload();
                        opendxp.helpers.openObject(responseJson.id, responseJson.type);

                        opendxp.elementservice.refreshNodeAllTrees("object", this.element.id);
                    } else {
    ...
    doDeleteVariant: function (id, answer) {
        if (answer == "yes") {
            if (opendxp.globalmanager.exists("object_" + id)) {
                var tabPanel = Ext.getCmp("opendxp_panel_tabs");
                tabPanel.remove("object_" + id);
            }

            Ext.Ajax.request({
                url: Routing.generate('opendxp_admin_dataobject_dataobject_delete'),
                method: 'DELETE',
                params: {
                    id: id
                },
...
opendxp.object.variantsTab.addMethods(opendxp.element.helpers.gridColumnConfig);
