...
opendxp.registerNS("opendxp.object.tags.localizedfields");
/**
 * @private
 */
opendxp.object.tags.localizedfields = Class.create(opendxp.object.tags.abstract, {
    ...
    initialize: function (data, fieldConfig) {
        ...
        if (opendxp.currentuser.admin || fieldConfig.permissionView === undefined || fieldConfig.permissionView === null) {
    ...
    getLayoutEdit: function (showMode) {
        ...
        if (this.fieldConfig.provideSplitView) {
            configureSplitViewButton = new Ext.Button({
                ...
                iconCls: 'opendxp_icon_side_by_side',
        ...
        if (isSplitViewEnabled && this.fieldConfig.provideSplitView) {
            ...
            if (existingSettings) {
                for (var currentLanguage in existingSettings.side) {
                    ...
                    if (existingSettings.side.hasOwnProperty(currentLanguage)) {
                        ...
                        var editable = (opendxp.currentuser.admin ||
            ...
            if (this.dropdownLayout) {
                ...
                for (var i = nrOfLanguages - 1; i >= 0; i--) {
                    ...
                    var editable = !showMode && (opendxp.currentuser.admin ||
                        this.fieldConfig.permissionEdit === undefined || this.fieldConfig.permissionEdit === null || this.fieldConfig.permissionEdit.length == 0 || in_array(currentLanguage, this.fieldConfig.permissionEdit));
    ...
    styleLanguageTab: function(item, hideLabels, language) {
        ...
        var languageTitle = t(opendxp.available_languages[language]);
        if (hideLabels) {
            item.title = '<div class="opendxp_icon_language_' + language.toLowerCase() + '" title="' + languageTitle + '" style="width: 20px; height:20px;"></div>';
            ...
            item.iconCls = "opendxp_icon_language_" + language.toLowerCase();
    ...
    getLocalStorageKey: function (legacyKey = true) {
        let key = "opendxp_lfSplitView_" + this.object.data.general.className;
        if(legacyKey === false) {
            key = key + "_" + opendxp.currentuser.id;
        }
        return key;
    },
            ...
            columns: [
                {
                    ...
                    renderer: function (value, metaData, record, row, col, store, gridView) {
                        return t(opendxp.available_languages[value]);
                    }
                }, {
                    xtype: 'checkcolumn',
                    text: t('left'),
...
opendxp.object.tags.localizedfields.addMethods(opendxp.object.helpers.edit);
