...
opendxp.registerNS("opendxp.object.tags.abstract");
/**
 * @private
 */
opendxp.object.tags.abstract = Class.create({
    ...
    addInheritanceSourceButton:function (metaData) {
        ...
        if (el && el.getFirstChild()) {
            ...
            el.insertHtml("afterBegin", '<div class="opendxp_open_inheritance_source"></div>');
            var button = Ext.get(el.query(".opendxp_open_inheritance_source")[0]);
            if (button) {
                button.addListener("click", function (metaData) {
                    ...
                    if (metaData) {
                        opendxp.helpers.openObject(metaData.objectid, "object");
                    }
    ...
    setGlobalLanguage: function () {
        if ((opendxp.globalmanager.get('global_language_' + this.getContext().objectId)) && this.frontendLanguages.includes(opendxp.globalmanager.get('global_language_' + this.getContext().objectId))) {
            this.globalLanguage = opendxp.globalmanager.get('global_language_' + this.getContext().objectId);
        }
    },
