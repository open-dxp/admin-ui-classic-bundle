...
opendxp.registerNS("opendxp.object.tags.advancedManyToManyRelation");
/**
 * @private
 */
opendxp.object.tags.advancedManyToManyRelation = Class.create(opendxp.object.tags.abstractRelations, {
    ...
    createLayout: function (readOnly) {
            ...
            if (this.fieldConfig.columns[i].type == "number") {
                ...
                if(!readOnly) {
                    cellEditor = function (fieldInfo) {
                        return new opendxp.object.helpers.metadataMultiselectEditor({
                            fieldInfo: fieldInfo
                        });
            ...
            items: [
                {
                    ...
                    handler: function (grid, rowIndex) {
                        ...
                        opendxp.helpers.openElement(data.data.id, data.data.type, subtype);
        ...
        if (this.fieldConfig.assetInlineDownloadAllowed) {
            columns.push({
                ...
                items: [
                    {
                        tooltip: t('download'),
                        icon: "/bundles/opendxpadmin/img/flat-color-icons/download-cloud.svg",
                        handler: function (grid, rowIndex) {
                            const data = grid.getStore().getAt(rowIndex);
                            if (data.data.id && data.data.type && data.data.type === "asset") {
                                opendxp.helpers.download(Routing.generate('opendxp_admin_asset_download', {id: data.data.id}));
                            }
                        }.bind(this)
                    }
                ]
        ...
        this.component = Ext.create('Ext.grid.Panel', {
            ...
            bodyCls: "opendxp_object_tag_objects opendxp_editable_grid",
            plugins: [
                this.cellEditing,
                'gridfilters'
            ]
    ...
    uploadDialog: function () {
        ...
        opendxp.helpers.assetSingleUploadDialog(this.fieldConfig.assetUploadPath, "path", function (res) {
            try {
    ...
    getGridColumnConfig: function (field) {
        return {
            ...
            renderer: opendxp.object.helpers.grid.prototype.advancedRelationGridRenderer.bind(this, field, "path"),
        };
    },
