...
opendxp.registerNS("opendxp.object.tags.hotspotimage");
/**
 * @private
 */
opendxp.object.tags.hotspotimage = Class.create(opendxp.object.tags.image, {
            ...
            renderer: function (key, value, metaData, record, rowIndex, colIndex, store, view) {
                ...
                if (value && value.id) {
                    ...
                    if (forGridConfigPreview) {
                        ...
                        return '<img src="' + Routing.generate('opendxp_admin_asset_getimagethumbnail', params)  + '" loading="lazy" />';
                    } else {
                        params['width'] = 88;
                        params['height'] = 88;
                        params['frame'] = true;

                        var url = Routing.generate('opendxp_admin_asset_getimagethumbnail', params);
    ...
    getToolbarItems: function () {
        ...
        if (this.fieldConfig.noteditable) {
            items.push({
                xtype: "tbtext",
                text: '<b><span class="opendxp_object_label_icon opendxp_icon_gray_lock"></span></b>'
            });
            items.push("->");
            items.push({
                xtype: "button",
        ...
        if (!this.additionalConfig.condensed) {
            items.push({
                ...
                width: 48,
                height: 24,
                cls: "opendxp_icon_droptarget_upload"
            });
        ...
        items.push({
            ...
            tooltip: t("crop"),
            overflowText: t('crop'),
            iconCls: "opendxp_icon_image_region",
        ...
        items.push({
            ...
            iconCls: "opendxp_icon_marker opendxp_icon_overlay_edit",
        ...
        items.push({
            ...
            iconCls: "opendxp_icon_marker opendxp_icon_overlay_delete",
    ...
    createImagePanel: function () {
        this.panel = new Ext.Panel({
            ...
            cls: "opendxp_droptarget_image",
            bodyCls: "opendxp_droptarget_image opendxp_image_container",
        ...
        this.panel.on("afterrender", function (el) {
            ...
            if (!this.fieldConfig.noteditable) {
                ...
                opendxp.helpers.registerAssetDnDSingleUpload(el.getEl().dom, this.fieldConfig.uploadPath, 'path', function (e) {
    ...
    updateImage: function () {
        ...
        body.removeCls("opendxp_droptarget_image");
        body.repaint();
    ...
    openCropWindow: function () {
        var editor = new opendxp.element.tag.imagecropper(this.data.id, this.crop, function (data) {
            this.crop = {};
    ...
    openHotspotWindow: function () {
        if (this.data) {
            var editor = new opendxp.element.tag.imagehotspotmarkereditor(
                this.data.id,
                {
    ...
    clearData: function () {
        this.doClearData();
        this.updateImage();
        opendxp.helpers.showNotification(t("success"), t("hotspots_cleared"), "success");
        ...
        if (this.fileinfo) {
            ...
            if (this.hotspots) {
                for (i = 0; i < this.hotspots.length; i++) {
                    ...
                    this.panel.body.insertHtml("beforeEnd", '<div id="' + hotspotId + '" class="opendxp_image_hotspot"></div>');
            ...
            if (this.marker) {
                for (i = 0; i < this.marker.length; i++) {
                    ...
                    this.panel.body.insertHtml("beforeEnd", '<div id="' + markerId + '" class="opendxp_image_marker"></div>');
