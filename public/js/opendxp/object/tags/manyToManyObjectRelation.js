...
opendxp.registerNS("opendxp.object.tags.manyToManyObjectRelation");
/**
 * @private
 */
opendxp.object.tags.manyToManyObjectRelation = Class.create(opendxp.object.tags.abstractRelations, {
    ...
    initialize: function (data, fieldConfig) {
        ...
        if (opendxp.helpers.hasSearchImplementation() && this.fieldConfig.displayMode === 'combo') {
            storeConfig.proxy = {
                type: 'ajax',
                url: opendxp.helpers.getObjectRelationInlineSearchRoute(),
                extraParams: {
    ...
    addCreateObject: function (className) {
        ...
        var classStore = opendxp.globalmanager.get("object_types_store");
        ...
        if (!invalid) {
            Ext.Ajax.request({
                ...
                params: {
                    ...
                    classId: classId,
                    parentId: parentId,
                    key: opendxp.helpers.getValidFilename(name, "object")
                },
                success: function (response) {
                    ...
                    if (data.success) {
                        ...
                        opendxp.helpers.openElement(data.id, "object", "object");

                        this.window.close();
                    } else {
                        opendxp.helpers.showNotification(t("error"), t("saving_failed"), "error", data.message);
                    }

                }.bind(this)
            });
        } else {
            opendxp.helpers.showNotification(t("error"), t("mandatory_field_empty"), "error");
        }
    },
    ...
    getVisibleColumns: function () {
        ...
        for (i = 0; i < visibleFields.length; i++) {
            if (!empty(this.fieldConfig.visibleFieldDefinitions) && !empty(visibleFields[i])) {
                ...
                var fc = opendxp.object.tags[layout.fieldtype].prototype.getGridColumnConfig(field);
    ...
    getLayoutEdit: function () {
        ...
        if (opendxp.helpers.hasSearchImplementation() && this.fieldConfig.displayMode === 'combo') {
            ...
            columns.push({
                ...
                items: [
                    {
                        tooltip: t('down'),
                        icon: "/bundles/opendxpadmin/img/flat-color-icons/down.svg",
                        handler: function (grid, rowIndex) {
        ...
        columns.push({
            ...
            items: [
                {
                    ...
                    handler: function (grid, rowIndex) {
                        ...
                        opendxp.helpers.deleteConfirm(t('relation'), data.data.path, function () {
                            grid.getStore().removeAt(rowIndex);
                        }.bind(this));
                    }.bind(this)
                }
            ...
            this.component = Ext.create('Ext.grid.Panel', {
                ...
                tbar: {
                    items: toolbarItems,
                    ctCls: "opendxp_force_auto_width",
                    cls: "opendxp_force_auto_width"
                },
        ...
        if (!readOnly) {
            if (this.fieldConfig.allowToClearRelation) {
                toolbarItems.push({
                    ...
                    tooltip: t("empty"),
                    handler: function () {
                        opendxp.helpers.deleteConfirm(t('all'), t('relations'), function () {
                            this.empty();
                        }.bind(this));
                    }.bind(this)
                });
    ...
    openSearchEditor: function () {
        ...
        opendxp.helpers.itemselector(true, this.addDataFromSelector.bind(this), {
                type: ["object"],
                subtype: {
    ...
    requestNicePathData: function (targets, isInitialLoad) {
        ...
        var nicePathRequested = opendxp.helpers.requestNicePathData(
            {
                type: "object",
                id: this.object.id
            },
            ...
            this.fieldConfig,
            context,
            opendxp.helpers.requestNicePathDataGridDecorator.bind(this, this.component.getView()),
            opendxp.helpers.getNicePathHandlerStore.bind(this, this.store, {
                idProperty: this.idProperty,
    ...
    loadObjectData: function (item, fields) {
        ...
        Ext.Ajax.request({
            url: Routing.generate('opendxp_admin_dataobject_dataobjecthelper_loadobjectdata'),
            params: {
                id: item.id,
                'fields[]': fields
            },
