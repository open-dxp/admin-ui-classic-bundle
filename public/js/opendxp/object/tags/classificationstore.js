...
opendxp.registerNS("opendxp.object.tags.classificationstore");
/**
 * @private
 */
opendxp.object.tags.classificationstore = Class.create(opendxp.object.tags.abstract, {
    ...
    initialize: function (data, fieldConfig) {
        ...
        if (this.fieldConfig.localized) {
            if (opendxp.currentuser.admin || fieldConfig.permissionView === undefined || fieldConfig.permissionView === null) {
                this.frontendLanguages = opendxp.settings.websiteLanguages.slice(0);
    ...
    getLayoutEdit: function () {
        ...
        var wrapperConfig = {
            bodyCls: "opendxp_object_tag_classification_store",
            border: true,
        ...
        if (this.dropdownLayout) {
            ...
            for (var i = 0; i < nrOfLanguages; i++) {
                ...
                let icon = "opendxp_icon_white_flag";

                if (currentLanguage !== "default") {
                    title = t(opendxp.available_languages[currentLanguage]);
                    icon = "opendxp_icon_language_" + currentLanguage.toLowerCase();
                }
    ...
    createGroupFieldset: function (language, group, groupedChildItems, isNew) {
        ...
        if (isNew) {
            config.cls = "opendxp_new_cs_group";
        }

        var fieldset = new Ext.create('opendxp.FieldSetTools', config);
    ...
    handleAddGroups: function (response) {
        ...
        if (
            ...
            opendxp.helpers.showNotification(t('validation_failed'), t('limit_reached'), 'error');
...
opendxp.object.tags.classificationstore.addMethods(opendxp.object.helpers.edit);
