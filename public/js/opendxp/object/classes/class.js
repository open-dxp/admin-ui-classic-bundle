...
opendxp.registerNS("opendxp.object.classes.klass");
/**
 * @private
 */
opendxp.object.classes.klass = Class.create({

    allowedInType: 'object',
    disallowedDataTypes: [],
    context: "class",
    uploadRoute: 'opendxp_admin_dataobject_class_importclass',
    exportRoute: 'opendxp_admin_dataobject_class_exportclass',
    ...
    addLayout: function () {
        ...
        panelButtons.push({
            ...
            iconCls: "opendxp_icon_class opendxp_icon_overlay_add",
            hidden: this instanceof opendxp.object.fieldcollections.field,
        ...
        this.panel = new Ext.Panel({
            ...
            autoScroll: true,
            title: name,
            //id: "opendxp_class_editor_panel_" + this.getId(),
    ...
    configureCustomLayouts: function() {
        try {
            new opendxp.object.helpers.customLayoutEditor(this.data);
        } catch (e) {
            console.log(e);
        }
    },
    ...
    upload: function() {
        ...
        opendxp.helpers.uploadDialog(this.getUploadUrl(), "Filedata", function() {
            Ext.Ajax.request({
                url: Routing.generate('opendxp_admin_dataobject_class_get'),
                params: {
                    id: this.data.id
                },
    ...
    getDataMenu: function(tree, record, allowedTypes, parentType, editMode) {
        ...
        for (var i = 0; i < dataComps.length; i++) {
            ...
            var dataComp = opendxp.object.classes.data[dataCompName];
            ...
            if (dataComps[i] != "data") { // class data is an abstract class => disallow
                if (in_array("data", allowedTypes[parentType]) || in_array(dataComps[i], allowedTypes[parentType]) ) {
                    ...
                    var group = opendxp.object.classes.data[dataComps[i]].prototype.getGroup();
                    if (!groups[group]) {
                    ...
                    groups[group].push({
                        text: opendxp.object.classes.data[dataComps[i]].prototype.getTypeName(),
                        iconCls: opendxp.object.classes.data[dataComps[i]].prototype.getIconClass(),
                        handler: handler
                    });
                }
            }
        ...
        for (i = 0; i < groupNames.length; i++) {
            if (groups[groupNames[i]] && groups[groupNames[i]].length > 0) {
                dataMenu.push(new Ext.menu.Item({
                    text: t(groupNames[i]),
                    iconCls: "opendxp_icon_data_group_" + groupNames[i],
                    hideOnClick: false,
    ...
    onTreeNodeContextmenu: function (tree, record, item, index, e, eOpts ) {
        ...
        var allowedTypes = opendxp.object.helpers.layout.getAllowedTypes(this);

        var dataComps = Object.keys(opendxp.object.classes.data);

        for (var i = 0; i < dataComps.length; i++) {
            var dataCompName = dataComps[i];
            if ('object' === typeof opendxp.object.classes.data[dataCompName]) {
                continue;
            }
            var component = opendxp.object.classes.data[dataCompName];
        ...
        if (childrenAllowed || changeTypeAllowed) {
            ...
            var layoutMenu = [];
            var layouts = Object.keys(opendxp.object.classes.layout);

            for (var i = 0; i < layouts.length; i++) {
                if (layouts[i] != "layout") {
                    if (in_array(layouts[i], allowedTypes[parentType])) {
                        layoutMenu.push({
                            text: opendxp.object.classes.layout[layouts[i]].prototype.getTypeName(),
                            iconCls: opendxp.object.classes.layout[layouts[i]].prototype.getIconClass(),
            ...
            if (childrenAllowed) {
                if (opendxp && opendxp.classEditor && opendxp.classEditor.clipboard) {
                    menu.add(new Ext.menu.Item({
                        text: t('paste'),
    ...
    cloneNode:  function(tree, node) {
        ...
        if (node.data.editor) {
            var definitions = newNode.data.editor = opendxp.object.classes[nodeType];
    ...
    copyNode: function(tree, record) {
        if (!opendxp.classEditor) {
            opendxp.classEditor = {};
        }

        var newNode = this.cloneNode(tree, record);
        opendxp.classEditor.clipboard = newNode;

    },

    dropNode: function(tree, record) {
        var node = opendxp.classEditor.clipboard;
    ...
    getRootPanel: function () {
        ...
        var getPhpClassName = function (name) {
            return "OpenDxp\\Model\\DataObject\\" + ucfirst(name);
        };
        ...
        this.rootPanel = new Ext.form.FormPanel({
            ...
            items: [
                ...
                opendxp.iconlibrary.createIconSelectionWidget(this.data.icon, this.getId()),
                {
                    xtype: "textfield",
                    fieldLabel: t("group"),
                    name: "group",
                ...
                },
                {
                    ...
                    cls: "opendxp_extra_label_headline"
                },
                {
                    xtype: "checkbox",
    ...
    getBricksGrid: function() {
        this.usagesStore = new Ext.data.ArrayStore({
            proxy: {
                url: Routing.generate('opendxp_admin_dataobject_class_getbrickusages'),
                type: 'ajax',
                reader: {
                    type: 'json'
                },
    ...
    addLayoutChild: function (type, initData, context) {
        ...
        var newNode = {
            ...
            type: "layout",
            iconCls: opendxp.object.classes.layout[type].prototype.getIconClass(),
            leaf: false,
            expandable: false,
            expanded: true,
        };
        ...
        var editor = new opendxp.object.classes.layout[type](newNode, initData);
    ...
    addDataChild: function (type, initData, context) {
        ...
        var newNode = {
            ...
            type: "data",
            leaf: true,
            iconCls: opendxp.object.classes.data[type].prototype.getIconClass()
        };
        ...
        var editor = new opendxp.object.classes.data[type](newNode, initData);
    ...
    changeDataType: function (tree, record, type, removeExisting, context) {
        try {
            ...
            var editor = new opendxp.object.classes.data[type](newNode, theData);
            editor.setContext(context);
        ...
        if (node.data.editor) {
            if (typeof node.data.editor.getData == "function") {
                ...
                if ((fieldValidation && in_arrayi(containerAwareDataName,this.usedFieldNames) == false) || data.name == "localizedfields" && data.fieldtype == "localizedfields") {
                    ...
                    opendxp.helpers.showNotification(t("error"), t("some_fields_cannot_be_saved"), "error",
                        invalidFieldsText);
    ...
    save: function () {
        ...
        if (this.data["name"].length > 2 &&
            ...
            if (this.getDataSuccess) {
                Ext.Ajax.request({
                    url: Routing.generate('opendxp_admin_dataobject_class_save'),
                    method: "PUT",
                    params: {
                        configuration: m,
                        values: n,
    ...
    saveOnComplete: function (response) {
        ...
        try {
            ...
            if(res.success) {
                ...
                opendxp.globalmanager.get("object_types_store").load();
                opendxp.globalmanager.get("object_types_store_create").load();
