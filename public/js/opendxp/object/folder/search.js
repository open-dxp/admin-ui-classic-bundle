...
opendxp.registerNS("opendxp.object.search");
/**
 * @private
 */
opendxp.object.search = Class.create(opendxp.object.helpers.gridTabAbstract, {
    systemColumns: ["id", "fullpath", "type", "subtype", "filename", "classname", "creationDate", "modificationDate"],
    fieldObject: {},
    gridType: 'object',

    title: t('search_edit'),
    icon: "opendxp_material_icon_search opendxp_material_icon",
    ...
    createGrid: function (fromConfig, response, settings, save, context) {
        ...
        const preCreateObjectGrid = new CustomEvent(opendxp.events.preCreateObjectGrid, {
            detail: {
                eventData: eventData
            },
            cancelable: true
        ...
        var gridHelper = new opendxp.object.helpers.grid(
            klass.data.text,
            fields,
            Routing.generate('opendxp_admin_dataobject_dataobject_gridproxy', eventData.requestParams),
            baseParams,
            false
        );
        ...
        if (needGridFilter) {
            plugins.push('opendxp.gridfilters');
        }
    ...
    onRowContextmenu: function (grid, record, tr, rowIndex, e, eOpts) {
        ...
        if (selectedRows.length <= 1) {
            ...
            if (opendxp.elementservice.showLocateInTreeButton("object")) {
                menu.add(new Ext.menu.Item({
                    ...
                    handler: function () {
                        try {
                            try {
                                opendxp.treenodelocator.showInTree(record.id, "object", this);
                            } catch (e) {
                                console.log(e);
                            }
            ...
            menu.add(new Ext.menu.Item({
                ...
                handler: function (data) {
                    var selectedRows = grid.getSelectionModel().getSelection();
                    for (var i = 0; i < selectedRows.length; i++) {
                        opendxp.helpers.openObject(selectedRows[i].data.id, "object");
                    }
                }.bind(this, data)
            }));
        ...
        const prepareOnRowContextmenu = new CustomEvent(opendxp.events.prepareOnRowContextmenu, {
            detail: {
                menu: menu,
                grid: this,
...
opendxp.object.search.addMethods(opendxp.element.helpers.gridColumnConfig);
