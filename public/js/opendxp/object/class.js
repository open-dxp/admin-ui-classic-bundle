...
opendxp.registerNS("opendxp.object.klass");
/**
 * @private
 */
opendxp.object.klass = Class.create({
    ...
    getTabPanel: function () {
        ...
        if (!this.panel) {
            ...
            tabPanel.setActiveItem("opendxp_classes");


            this.panel.on("destroy", function () {
                opendxp.globalmanager.remove("classes");
            }.bind(this));
    ...
    suggestIdentifier: function() {
        Ext.Ajax.request({
            url: Routing.generate('opendxp_admin_dataobject_class_suggestclassidentifier'),
            params: {
            },
            ...
            this.editPanel = new Ext.TabPanel({
                ...
                listeners: {
                    'tabchange': function (tabpanel, tab) {
                        var classId = tab.id.substr("opendxp_class_editor_panel_".length);
                        var tree = this.tree;
    ...
    openClass: function (id) {
        if (Ext.getCmp("opendxp_class_editor_panel_" + id)) {
            this.getEditPanel().setActiveTab(Ext.getCmp("opendxp_class_editor_panel_" + id));
            return;
        }
    ...
    addClassPanel: function (response) {
        ...
        var classPanel = new opendxp.object.classes.klass(data, this, this.openClass.bind(this, data.id), "opendxp_class_editor_panel_");
        opendxp.layout.refresh();
    },
    ...
    onTreeNodeContextmenu: function (tree, record, item, index, e, eOpts) {
        ...
        menu.add(new Ext.menu.Item({
            text: t('delete'),
            iconCls: "opendxp_icon_class opendxp_icon_overlay_delete",
    ...
    addClassComplete: function (className, classIdentifier, classes) {
        ...
        Ext.Ajax.request({
            url: Routing.generate('opendxp_admin_dataobject_class_add'),
            method: "POST",
            params: {
                className: className,
            ...
            success: function (response) {
                ...
                opendxp.globalmanager.get("object_types_store").reload();
                opendxp.globalmanager.get("object_types_store_create").reload();
    ...
    deleteClass: function (tree, record) {
        ...
        Ext.Msg.confirm(t('delete'), sprintf(t('delete_message_advanced'), t('class'), record.data.text), function (btn) {
            if (btn == 'yes') {
                Ext.Ajax.request({
                    url: Routing.generate('opendxp_admin_dataobject_class_delete'),
                    method: 'DELETE',
                    params: {
                        id: record.data.id
                    },
    ...
    activate: function () {
        Ext.getCmp("opendxp_panel_tabs").setActiveItem("opendxp_classes");
    }
